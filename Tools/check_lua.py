...
def toc_lua_files(toc_name="ToonAge.toc"):
    """The .lua files the game actually loads, in load order, for one TOC."""
    toc = REPO_ROOT / toc_name
    if not toc.exists():
        return []
    files = []
...
def main():
    # --toc <name> selects which TOC's file list to check (default ToonAge.toc).
    toc_name = "ToonAge.toc"
    argv = sys.argv[1:]
    if "--toc" in argv:
        i = argv.index("--toc")
        if i + 1 < len(argv):
            toc_name = argv[i + 1]
            # Drop the flag + its value from positional args.
    ...
    if args:
        ...
        files = toc_lua_files(toc_name)
        label = f"{len(files)} files listed in {toc_name}"
