...
HEADERS  = {"User-Agent": "Mozilla/5.0 (ToonAge/1.0 guide-stub-generator)"}
...
def generate_lua(quests, guide_id, title, zone_map_id=0, min_level=1, max_level=60):
    lines = []
    lines.append(f"-- ToonAge/Data/Guides/TAG_{guide_id}.lua")
    lines.append( "-- STUB -- auto-generated by generate_guide_stub.py")
    lines.append( "-- TODO: fill in zone map ID, coords (/coord), step text.")
    lines.append( "")
    lines.append( "local TA = ToonAge")
    lines.append( "TA.GuideData = TA.GuideData or {}")
    lines.append( "")
    lines.append(f'TA.GuideData["{guide_id.lower()}"] = {{')
...
CHAINS = [
    ...
    #     "max_level":   80,
    #     "ptr":         True,
    #     "output":      "TAG_Midnight_QuelsThelas.lua",
    # },
]
...
if __name__ == "__main__":
    ...
    for chain in CHAINS:
        ...
        out = chain.get("output") or (ensure(GUIDES_DIR) / f"TAG_{chain['guide_id']}.lua")
        with open(out, "w", encoding="utf-8") as f:
            f.write(lua)

        print(f"\n  {len(quests)} quest stubs written to {out}")
        print(f"\n  Next steps:")
        print(f"    1. Add the filename to ToonAge.toc before GuideParser.lua")
        print(f"    2. /reload in-game -- tracker shows the stub steps")
        print(f"    3. Stand at each accept NPC and run /coord to fill coords")
        print(f"    4. Replace stub text with real guide prose as you play through")
