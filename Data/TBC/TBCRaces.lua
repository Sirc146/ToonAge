...
TA.Data.RaceMechanics = {
    ...
    Draenei = {
        -- The Draenei hit racial is CLASS-DEPENDENT in TBC (merged only in WotLK):
        --   Heroic Presence   — +1% melee/ranged hit: Warrior, Paladin, Hunter
        --   Inspiring Presence — +1% spell hit:       Mage, Priest, Shaman
        -- (Wowhead TBC spells 6562 / 28878; class split per lootxphub's TBC racial
        -- guide.) Both are party-wide auras and neither shows in
        -- GetCombatRatingBonus, so the cap maths adds the one this class has.
        -- The previous table gave every Draenei BOTH, which told a Draenei
        -- Enhancement Shaman they needed 1% less melee hit than they do.
        byClass = {
            WARRIOR = { meleeHitPercent = 1, source = "Heroic Presence" },
            PALADIN = { meleeHitPercent = 1, source = "Heroic Presence" },
            HUNTER  = { meleeHitPercent = 1, source = "Heroic Presence" },
            MAGE    = { spellHitPercent = 1, source = "Inspiring Presence" },
            PRIEST  = { spellHitPercent = 1, source = "Inspiring Presence" },
            SHAMAN  = { spellHitPercent = 1, source = "Inspiring Presence" },
        },
        source = "Heroic Presence / Inspiring Presence",
    },
    NightElf = {
        dodgePercent = 1,
        source = "Quickness",
...
TA.Data.RaceTraits = {
    Human = {
        ...
        traits = {
            ...
            { n = "The Human Spirit",     d = "+5% Spirit — more out-of-combat and Five Second Rule regen" },
    ...
    Draenei = {
        name = "Draenei",
        traits = {
            { n = "Heroic Presence / Inspiring Presence", d = "+1% hit (Warrior, Paladin, Hunter) or +1% spell hit (Mage, Priest, Shaman) for you and your party", combat = true },
...
--- Racial mechanics for a race, with any class-specific entry merged in.
--- @param raceToken string  UnitRace() token
--- @param classToken string|nil  UnitClass() token (defaults to the player)
function TA.Data.GetRaceMechanics(raceToken, classToken)
    local m = TA.Data.RaceMechanics[raceToken]
    if not m or not m.byClass then return m end
    if not classToken then
        local _, c = UnitClass and UnitClass("player")
        classToken = c
    end
    local merged = {}
    for k, v in pairs(m) do if k ~= "byClass" then merged[k] = v end end
    local extra = classToken and m.byClass[classToken]
    if extra then for k, v in pairs(extra) do merged[k] = v end end
    return merged
end
