...
TA.Data.RoleWeights = {
    ...
    -- Agility melee: Rogues get 1 AP plus crit and dodge per Agility but only
    -- 1 AP per Strength. Icy Veins' TBC Feral guide ranks Agility first for
    -- cats too (Strength is worth 2.4 AP with Heart of the Wild, so it sits
    -- closer). Enhancement Shaman stays on the Strength table: Icy Veins ranks
    -- Strength well above Agility for them.
    MELEE_AGI = {
        primary = "AGI",
        AGI = 1.00, STR = 0.55, AP = 0.45, STA = 0.10,
        HIT = 1.20, EXP = 1.15, CRIT = 0.80, HASTE = 0.75,
        ARMOR = 0.01, RESIL = 0.20,
        note = "Agility-based melee (Rogue). Hit and expertise lead until both are capped.",
    },
    MELEE_CAT = {
        primary = "AGI",
        AGI = 1.00, STR = 0.80, AP = 0.40, STA = 0.10,
        HIT = 1.20, EXP = 1.15, CRIT = 0.75, HASTE = 0.70,
        ARMOR = 0.01, RESIL = 0.20,
        note = "Feral cat. Agility first; Strength close behind with Heart of the Wild. "
            .. "Watch the crit soft cap (54.4% minus miss chance).",
    },
    RANGED = {
        primary = "AGI",
...
TA.Data.PvPRoleWeights = {
    ...
    MELEE_AGI = {
        primary = "AGI",
        AGI = 1.00, STR = 0.55, AP = 0.45, STA = 0.85,
        HIT = 0.90, EXP = 0.35, CRIT = 0.70, HASTE = 0.65,
        RESIL = 1.20, ARMOR = 0.02,
        note = "Resilience and Stamina lead; Agility is still your damage stat.",
    },
    MELEE_CAT = {
        primary = "AGI",
        AGI = 1.00, STR = 0.80, AP = 0.40, STA = 0.85,
        HIT = 0.90, EXP = 0.35, CRIT = 0.65, HASTE = 0.60,
        RESIL = 1.20, ARMOR = 0.02,
        note = "Resilience and Stamina lead; Agility then Strength for damage.",
    },
    RANGED = {
        primary = "AGI",
...
function TA.Data.GetWeights(role, pvp)
    ...
    -- One MELEE table used to serve every melee class, Strength-first, so
    -- Rogues and Feral cats had Agility undervalued. Pick by class.
    if role == "MELEE" then
        local class = ToonAge.Utils and ToonAge.Utils.GetPlayerClass and ToonAge.Utils.GetPlayerClass()
        if class == "ROGUE" and set.MELEE_AGI then return set.MELEE_AGI end
        if class == "DRUID" and set.MELEE_CAT then return set.MELEE_CAT end
    end
