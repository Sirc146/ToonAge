...
-- ── Zygor companion mode ──────────────────────────────────────────────
-- Zygor accepts/turns in quests, picks rewards, runs its own arrow and can
-- equip gear. If ToonAge does the same things at the same time the two fight
-- (double reward picks, gear swapping back and forth). When Zygor is loaded
-- ToonAge steps back from those jobs unless the player turns this off.
-- Zygor's global table is ZygorGuidesViewer on every flavor; the folder
-- names are checked too in case the global is renamed.
local ZYGOR_FOLDERS = {
...
function U.ZygorLoaded()
    if rawget(_G, "ZygorGuidesViewer") then return true end
    for _, name in ipairs(ZYGOR_FOLDERS) do
        if U.IsAddOnLoaded(name) then return true end
    end
    return false
end

--- True when ToonAge should leave questing automation, the arrow and
--- auto-equip to Zygor. Setting: charDB.tracker.deferToZygor (nil = on).
function U.DeferToZygor()
    local t = TA.charDB and TA.charDB.tracker
    if t and t.deferToZygor == false then return false end
    return U.ZygorLoaded()
end
...
--- Retail item stats in ToonAge's stat keys.
--- C_Item.GetItemStats keys follow the global strings, and Versatility is the odd
--- one out: its key is ITEM_MOD_VERSATILITY, with no _SHORT suffix (Warcraft Wiki,
--- API_C_Item.GetItemStats). Reading ITEM_MOD_VERSATILITY_SHORT always returned
--- nil, so every point of Versatility on gear scored as zero -- a 28-Versatility
--- neck lost to an item-level-26 Heart of Azeroth.
--- @return table stats { INT, AGI, STR, STAM, CRIT, HASTE, MASTERY, VERS }
local function StatNum(v)
    if v == nil then return 0 end
    if type(v) == "number" then return v end
    return tonumber(tostring(v):match("([%d%.%-]+)")) or 0
end
function U.ReadItemStats(itemLink)
    local raw = (itemLink and C_Item and C_Item.GetItemStats and C_Item.GetItemStats(itemLink))
             or (itemLink and GetItemStats and GetItemStats(itemLink)) or {}
    return {
        INT     = StatNum(raw["ITEM_MOD_INTELLECT_SHORT"]),
        AGI     = StatNum(raw["ITEM_MOD_AGILITY_SHORT"]),
        STR     = StatNum(raw["ITEM_MOD_STRENGTH_SHORT"]),
        STAM    = StatNum(raw["ITEM_MOD_STAMINA_SHORT"]),
        CRIT    = StatNum(raw["ITEM_MOD_CRIT_RATING_SHORT"]),
        HASTE   = StatNum(raw["ITEM_MOD_HASTE_RATING_SHORT"]),
        MASTERY = StatNum(raw["ITEM_MOD_MASTERY_RATING_SHORT"]),
        VERS    = StatNum(raw["ITEM_MOD_VERSATILITY"] or raw["ITEM_MOD_VERSATILITY_SHORT"]),
    }
end
...
--- @return number averageIlvl, number slotsCounted
--- The sidebar prints "(N slots)"; this used to return only the average, so it
--- always read "(0 slots)".
function U.GetAverageIlvl()
    local counted, total = 0, 0
    for slot = 1, 18 do
        if slot ~= 4 then   -- shirt
            ...
            if link then
                ...
                total = total + (tonumber(ilvl) or 0)
            end
        end
    end
    if GetAverageItemLevel then
        local ok, _, equipped = pcall(GetAverageItemLevel)
        if ok and equipped then return math.floor(equipped), counted end
    end
    return (counted > 0) and math.floor(total / counted) or 0, counted
end
...
-- ══════════════════════════════════════════════════════════════════════════
-- ── Container (bag) delegators → Core/Compat/API.lua ──────────────────────
-- ══════════════════════════════════════════════════════════════════════════
-- The retail trunk never scanned bags (it reads equipped/inspect data), so
-- these did not exist here before. The classic-family builds (TBC/MoP) DO scan
-- bags — AutoEquip, Gear and PetCare all call U.GetContainerNumSlots. Rather
-- than each flavor re-detecting C_Container vs the bare globals, the detection
-- lives once in Core/Compat/API.lua and these are thin delegators so existing
-- call sites (U.GetContainerNumSlots, etc.) keep working unchanged in the
-- unified tree.
--
-- Guarded with `TA.Compat and ...` so Utils never hard-depends on Compat load
-- order; if Compat is somehow absent these degrade to a safe empty result
-- rather than erroring.

function U.GetContainerNumSlots(bag)
    if TA.Compat and TA.Compat.GetContainerNumSlots then
        return TA.Compat.GetContainerNumSlots(bag)
    end
    return 0
end

function U.GetContainerItemLink(bag, slot)
    if TA.Compat and TA.Compat.GetContainerItemLink then
        return TA.Compat.GetContainerItemLink(bag, slot)
    end
    return nil
end

function U.GetContainerItemID(bag, slot)
    if TA.Compat and TA.Compat.GetContainerItemID then
        return TA.Compat.GetContainerItemID(bag, slot)
    end
    return nil
end

function U.GetContainerItemInfo(bag, slot)
    if TA.Compat and TA.Compat.GetContainerItemInfo then
