...
function State:Register(key, owner, invalidatedBy)
    ...
    if BARRED_OWNERS[owner] then
        ...
        if TA.ErrorLog then
            TA.ErrorLog:Log("State:Register", msg, key)
        end
        return false
    end
    ...
    if existing and existing.owner ~= owner then
        ...
        local msg = ("key '%s' already owned by %s, refused claim by %s"):format(key, existing.owner, owner)
        if TA.ErrorLog then
            TA.ErrorLog:Log("State:Register", msg, "")
        end
        return false
    end
...
State.SlashCommands = {
    state = function(self)
        ...
        for _, key in ipairs(keys) do
            ...
            local events = {}
            for event in pairs(reg.events) do
            ...
            if e then
                TA:Raw(
                    TA.LOG.OUTPUT,
                    ("  |cFF4AFF7A●|r %s |cFF888780(%s, %.1fs old)|r"):format(key, reg.owner, GetTime() - e.stamp)
                )
            else
