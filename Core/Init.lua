...
function TA:Print(level, module, msg)
    ...
    if level > (TA.logLevel or TA.LOG.WARN) then
        return
    end
    print(
        string.format(
...
function TA:InitModules()
    ...
    if safe then
        TA:Print(
            TA.LOG.WARN,
            nil,
            "SAFE MODE — only core modules loaded. " .. "|cFF888780/ta safemode to turn off, then /reload.|r"
        )
    end
        ...
        if userDisabled or safeSkipped then
            ...
            if mod.Init then
                ...
                if not ok then
                    ...
                    if TA.ErrorLog then
                        TA.ErrorLog:Log(name .. " Init", tostring(err), "")
                    end
                end
            end
        end
            ...
            if not ok then
                ...
                elseif mod._errorCount == ERROR_PRINT_LIMIT + 1 then
                    TA:Printf(
                        TA.LOG.WARN,
                        nil,
                        "%s keeps failing — muting further errors. " .. "|cFF888780/ta errors to read them.|r",
                        name
                    )
                end
                ...
                if TA.ErrorLog then
                    TA.ErrorLog:Log(name .. " OnEvent", tostring(err), event or "")
                end
                ...
                if mod._errorCount >= ERROR_DISABLE_THRESHOLD then
                    ...
                    TA:Print(
                        TA.LOG.WARN,
                        "Safe Mode",
                        msg .. ". |cFF888780Reload to re-enable. /ta health for status.|r"
                    )
                    if TA.ErrorLog then
                        TA.ErrorLog:Log("SafeMode", msg, event or "")
                    end
                end
            end
        end
...
function TA:QueueUIRefresh(event)
    ...
    if not (self.UI and self.UI:IsVisible()) then
        return
    end
    ...
    C_Timer.After(UI_REFRESH_DELAY, function()
        ...
        if not ok then
            ...
            if TA.ErrorLog then
                TA.ErrorLog:Log("UI Refresh", tostring(err), "")
            end
        end
    end)
end
...
function TA:OnLogin()
    ...
    TA:Raw(
        TA.LOG.INFO,
        "|cFFFFD100ToonAge|r v"
            .. self.version
            .. " loaded. Type "
            .. self:MakeSlashLink("help", "/ta help")
            .. " for clickable commands."
    )
end
...
function TA:SlashCommand(msg)
        ...
        safemode = function()
            ...
            if self.db.safeMode then
                TA:Print(
                    TA.LOG.OUTPUT,
                    nil,
                    "Safe Mode |cFFFF9A1AON|r — next load initialises "
                ...
                TA:Print(
                    TA.LOG.OUTPUT,
                    nil,
                    "Safe Mode |cFF4AFF7AOFF|r. " .. "|cFF888780/reload to load everything again.|r"
                )
            end
        end,
        health = function()
            ...
            for _, entry in ipairs(report) do
                ...
                elseif entry.status == "errored" then
                    ...
                    TA:Raw(
                        TA.LOG.OUTPUT,
                        ("  |cFFFF4444✗ %s|r — init failed: %s"):format(entry.name, tostring(entry.error))
                    )
                else
                    off = off + 1
            ...
            TA:Raw(
                TA.LOG.OUTPUT,
                ("  |cFF4AFF7A%d loaded|r · |cFF888780%d off|r · |cFFFF4444%d errored|r"):format(loaded, off, errored)
            )
            ...
            if pending and pending > 0 then
                TA:Raw(
                    TA.LOG.OUTPUT,
                    ("  |cFF888780%d item request(s) awaiting GET_ITEM_INFO_RECEIVED|r"):format(pending)
                )
            end
            if self.db.safeMode then
    ...
    -- the quietest one.
    if cmd == "verbose" then
        local LEVELS = { error = TA.LOG.ERROR, warn = TA.LOG.WARN, info = TA.LOG.INFO, debug = TA.LOG.DEBUG }
        ...
        if not want then
            TA:Printf(
                TA.LOG.OUTPUT,
                nil,
                "Chat verbosity is |cFF4AFF7A%s|r.",
    ...
    if cmd == "toggle" then
        ...
        for name in pairs(self.db.modules or {}) do
            if name:lower() == modName:lower() then
                matchedKey = name
                break
            end
        end
...
function TA:FuzzyScore(input, candidate)
    if not input or not candidate then
        return 0
    end
...
function TA:PrintInteractiveHelp()
    ...
    TA:Raw(
        TA.LOG.OUTPUT,
        "    "
            .. self:MakeSlashLink("prof", "Professions")
            .. "  "
    ...
    TA:Raw(
        TA.LOG.OUTPUT,
        "    "
            .. self:MakeSlashLink("tracker", "Toggle Tracker")
            .. "  "
    ...
    TA:Raw(
        TA.LOG.OUTPUT,
        "    "
            .. self:MakeSlashLink("diag", "Diagnose Tracker")
            .. "  "
    ...
    TA:Raw(
        TA.LOG.OUTPUT,
        "    " .. self:MakeSlashLink("arrow", "Toggle Arrow") .. "  " .. self:MakeSlashLink("hud", "Toggle NavHud")
    )
    TA:Raw(
        TA.LOG.OUTPUT,
        "    "
            .. self:MakeSlashLink("trail", "Toggle AntTrail")
            .. "  "
    ...
    TA:Raw(
        TA.LOG.OUTPUT,
        "    " .. self:MakeSlashLink("coord", "Show Coordinates") .. "  " .. self:MakeSlashLink("xp", "XP Stats")
    )
    TA:Raw(
        TA.LOG.OUTPUT,
        "    " .. self:MakeSlashLink("alts", "Alt Roster") .. "  " .. self:MakeSlashLink("todo", "Weekly Todo")
    )
    TA:Raw(
        TA.LOG.OUTPUT,
        "    " .. self:MakeSlashLink("gather", "Gather History") .. "  " .. self:MakeSlashLink("errors", "Error Log")
    )
    TA:Raw(TA.LOG.OUTPUT, "")
    TA:Raw(TA.LOG.OUTPUT, "  |cFF888780SYSTEM:|r")
    TA:Raw(
        TA.LOG.OUTPUT,
        "    " .. self:MakeSlashLink("options", "Settings") .. "  " .. self:MakeSlashLink("layout", "Toggle Layout")
    )
    TA:Raw(
        TA.LOG.OUTPUT,
        "    " .. self:MakeSlashLink("toggle", "Module Toggles") .. "  " .. self:MakeSlashLink("reset", "Reset Data")
    )
    TA:Raw(TA.LOG.OUTPUT, "")
