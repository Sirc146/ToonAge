...
-- Bindings.xml names a header and one binding token per entry; without these
-- globals the Key Bindings UI shows the raw tokens instead of readable text.
-- Bindings.xml previously also carried an invalid `category` attribute that
-- isn't part of the documented Binding element schema — that's what caused
-- "Unrecognized XML: Binding" on login; it's been removed from the XML.
BINDING_HEADER_TOONAGE              = "ToonAge"
BINDING_NAME_TOONAGE_TOGGLE_PANEL   = "Toggle ToonAge panel"
BINDING_NAME_TOONAGE_TOGGLE_NAVHUD  = "Toggle NavHud"
BINDING_NAME_TOONAGE_TOGGLE_ARROW   = "Toggle Arrow"
BINDING_NAME_TOONAGE_TOGGLE_TRACKER = "Toggle Quest Tracker"
