...
function TA:InitUI()
    ...
    optionsBtn:SetScript("OnEnter", function()
        optIcon:SetTextColor(0.92, 0.90, 0.87, 1)
    end)
    ...
    function frame:SetTab(tabID)
        ...
        for id, btn in pairs(self.tabButtons) do
            ...
            if isActive then
                btn.label:SetTextColor(0.92, 0.90, 0.87, 1.00)
            else
                btn.label:SetTextColor(0.55, 0.52, 0.48, 1.00)
            end
            if isActive then
                btn.activeLine:Show()
            else
    ...
    function frame:Refresh(events)
        ...
        if events then
            ...
            if not BatchAffectsTab(events, self.activeTab) then
                return
            end
        end
    ...
    function frame:RebuildTabs()
        ...
        if not IsTabEnabled(tabToShow) then
            tabToShow = "character"
        end
...
local function PositionArrow(arrowF, defaultPos, dx, dy)
    if not arrowF or not defaultPos then
        return
    end
    ...
    if saved and saved.x and saved.y then
        ...
        arrowF:SetPoint(
            ...
            defaultPos.x + (dx or 0),
            defaultPos.y + (dy or 0)
        )
    end
