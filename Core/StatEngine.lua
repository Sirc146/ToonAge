...
-- ── Level scaling of the rating bounds ────────────────────────────────
-- DR is defined in EFFECT PERCENT (first bracket = 30%), but DR_BRACKETS holds
-- level-90 RATING numbers. While leveling, rating-per-% is far lower, so the
-- same 30% arrives at a much smaller rating. Comparing a level-40 character's
-- rating against the L90 bound 1320 meant DR never applied below max level.
-- Scale every bound by (live rating-per-% / L90 rating-per-%). At 90 the ratio
-- is ~1 and nothing changes; inside DR RatingPer1Pct already falls back to the
-- L90 constant, which also yields 1.
local function BoundScale(statKey)
    local ref = RATING_PER_PCT_L90[statKey]
    if not ref then return 1.0 end
    local live = RatingPer1Pct(statKey)
    if not live or live <= 0 then return 1.0 end
    local f = live / ref
    if f < 0.02 then f = 0.02 elseif f > 1.10 then f = 1.10 end
    return f
end

--- First DR bound for a secondary, in rating, at the player's CURRENT level.
function StatEngine:GetFirstDRRating(statKey)
    local brackets = DR_BRACKETS[statKey]
    if not brackets then return nil end
    return math.floor(brackets[1][1] * BoundScale(statKey) + 0.5)
end

function StatEngine:GetPenalty(statKey, rating)
    local brackets = DR_BRACKETS[statKey]
    if not brackets then return 0 end
    local scale = BoundScale(statKey)
    for _, b in ipairs(brackets) do
        if rating < b[1] * scale then return b[2] end
    end
...
function StatEngine:GetSpanRetention(statKey, fromRating, amount)
    ...
    local scale = BoundScale(statKey)
    for _, b in ipairs(brackets) do
        local bound, penalty = b[1] * scale, b[2]
...
function StatEngine:GetMarginalValue(statKey, specID, role)
    ...
    -- Spec breakpoints (StatWeights softCaps, level-90 rating): past it the stat
    -- is worth no more than the spec's weakest secondary (e.g. Devourer Haste
    -- above ~800, per Wowhead's "do not go much higher" guidance).
    local sw = SW and specID and SW[specID]
    local cap = sw and sw.softCaps and sw.softCaps[statKey]
    if cap and CR[statKey] and weights
        and SafeRating(CR[statKey]) >= cap * BoundScale(statKey) then
        local floorW = math.huge
        for _, k in ipairs({ "HASTE", "CRIT", "MASTERY", "VERS" }) do
            if weights[k] and weights[k] < floorW then floorW = weights[k] end
        end
        if floorW < baseWeight then baseWeight = floorW end
    end
...
--- @param without table|nil  stats of the item being replaced; subtracted from
---        the live rating before DR so both items start from the same baseline.
function StatEngine:ScoreItem(stats, specID, role, without)
    ...
    for statKey, amount in pairs(stats) do
        ...
        if a ~= 0 then
            ...
            local retention = 1.0
            if CR[statKey] then
                local from = SafeRating(CR[statKey]) - ((without and without[statKey]) or 0)
                if from < 0 then from = 0 end
                retention = self:GetSpanRetention(statKey, from, a)
            end
