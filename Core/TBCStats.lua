...
-- Other global names the same index is known by on newer Classic engines.
-- Resilience in particular is CR_RESILIENCE_CRIT_TAKEN / COMBAT_RATING_RESILIENCE_CRIT_TAKEN
-- there, so looking only for CR_CRIT_TAKEN_MELEE reported a false "absent".
local CR_ALIASES = {
    CRIT_TAKEN_MELEE = { "CR_RESILIENCE_CRIT_TAKEN", "COMBAT_RATING_RESILIENCE_CRIT_TAKEN" },
}

for key, fallback in pairs(CR_FALLBACK) do
    local named = _G["CR_" .. key]
    if type(named) ~= "number" and CR_ALIASES[key] then
        for _, alias in ipairs(CR_ALIASES[key]) do
            if type(_G[alias]) == "number" then named = _G[alias]; break end
        end
    end
...
--
-- Below level 60 the conversion comes from the community level formula (see
-- LevelFactor) and is reported with source "estimate" so the UI marks it
-- approximate. Percentage targets ("you need 4.20% more hit") never use it.
...
--- Level scaling of "rating per 1%" relative to level 60.
---   level 61-70: 82 / (262 - 3*level)     (reproduces every level-70 figure above)
---   level 10-60: (level - 8) / 52         (1.0 at 60, so the two pieces join)
---   below 10:     2 / 52
--- The 61-70 piece is confirmed by the level-70 numbers. The sub-60 piece is the
--- long-standing community formula (RatingBuster/WoWWiki era) and is NOT
--- confirmed on this client, so callers label it approximate. It only matters
--- for the "about N rating" hint; the percentage targets never use it.
function S:LevelFactor(level)
    ...
    if level >= 10 then return (level - 8) / 52 end
    return 2 / 52
end
...
function S:GetRatingPerPercent(key)
    ...
    return base * factor, (level < 60) and "estimate" or "formula"
end
...
function S:GetBonusHit()
    ...
    local racial = TA.Data and TA.Data.GetRaceMechanics and TA.Data.GetRaceMechanics(token)
    if racial then
    ...
    if charDB.hitBonusMelee or charDB.hitBonusSpell then
        ...
        sources = { { label = "your manual hit override", amount = nil,
...
function S:GetMeleeHitCap(opts)
    ...
    -- Ranged attacks read ranged hit rating (CR_HIT_RANGED), not melee.
    local fromGear = opts.ranged and self:GetRatingBonus("HIT_RANGED") or self:GetMeleeHitPercent()
...
function S:GetDefenseCap(opts)
    ...
    -- Other crit-taken reduction lowers the defense needed: talents (Survival of
    -- the Fittest) and resilience. Never below zero.
    local talentPct, talentFound = 0, {}
    if TA.Data and TA.Data.DetectCritTakenReduction then
        local okT, p, f = pcall(TA.Data.DetectCritTakenReduction)
        if okT and type(p) == "number" then talentPct, talentFound = p, f or {} end
    end
    local resilPct = 0
    local okR, resil = pcall(self.GetResilience, self)
    if okR and resil and type(resil.critReduction) == "number" then resilPct = resil.critReduction end
    local fromDefenseNeeded = math.max(critTaken - talentPct - resilPct, 0)

    local overBase  = math.ceil(fromDefenseNeeded / CRIT_REDUCTION_PER_DEFENSE - 1e-9)
    ...
    return {
        ...
        critTaken    = critTaken,
        critFromTalents = talentPct,
        talentSources   = talentFound,
        critFromResilience = resilPct,
        cap          = cap,
        baseDefense  = base,
...
function S:GetArmorInfo(opts)
    ...
    -- Fixed 2026-09-07: `UnitArmor and UnitArmor("player")` always yielded
    -- effArmor = nil, even when UnitArmor exists — `and`/`or` truncate the
    -- right-hand call to a single value in Lua 5.1, so the second return was
...
function S:ConfidenceReport()
    ...
    if self.crAnyGuessed then
        ...
        table.sort(guessed)
        lines[#lines + 1] = U.Orange("Using the standard TBC index for: ")
            .. table.concat(guessed, ", ") .. " |cFF888780(client has no named constant; values unaffected)|r"
    else
    ...
    local level = U.GetPlayerLevel()
    local perPct, src = self:GetRatingPerPercent("HIT_MELEE")
    local srcText = (src == "derived" and U.Green("read from your gear"))
                 or (src == "formula" and U.Orange("level formula (no hit rating on gear yet)"))
                 or (src == "estimate" and U.Orange("estimated for your level (no hit rating on gear yet)"))
                 or U.Red("unavailable")
    lines[#lines + 1] = perPct
        and string.format("Hit rating at level %d: about %.2f rating per 1%% — %s", level, perPct, srcText)
        or ("Hit rating conversion: " .. srcText)

    return lines
end

function S:Init()
    -- Index fallbacks are shown on the Stat Caps tab; no login chat warning.
end
