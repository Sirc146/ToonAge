...
function Scan:GetActiveWeaponSkill(ranged)
    if ranged then
        -- UnitRangedAttack -> base skill, modifier for the equipped ranged weapon.
        if UnitRangedAttack then
            local ok, base, mod = pcall(UnitRangedAttack, "player")
            base, mod = U.SafeNum(base), U.SafeNum(mod)
            if ok and base > 0 then return base + mod, true end
        end
