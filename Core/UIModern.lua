...
function M:ClearGlassBackdrop(frame)
    ...
    if frame._glassFrost then
        frame._glassFrost:Hide()
    end
    if frame._glassShadow then
        frame._glassShadow:Hide()
    end
end
...
function M:ClearGlowBorder(frame)
    if frame._modernGlow then
        frame._modernGlow:Hide()
    end
    if frame._modernGlowMask then
        frame._modernGlowMask:Hide()
    end
end
...
function M:ApplyStatusStrip(frame, status, height)
    ...
    if status == "ok" then
        strip:SetGradient("HORIZONTAL", CreateColor(0.15, 0.70, 0.30, 0.8), CreateColor(0.15, 0.70, 0.30, 0.0))
    elseif status == "upgrade" then
        strip:SetGradient("HORIZONTAL", CreateColor(0.12, 1.00, 0.00, 0.9), CreateColor(0.12, 1.00, 0.00, 0.0))
    elseif status == "warning" then
        strip:SetGradient("HORIZONTAL", CreateColor(1.00, 0.65, 0.10, 0.8), CreateColor(1.00, 0.65, 0.10, 0.0))
    elseif status == "danger" then
        strip:SetGradient("HORIZONTAL", CreateColor(1.00, 0.30, 0.25, 0.9), CreateColor(1.00, 0.30, 0.25, 0.0))
    else
        strip:Hide()
    end
end
...
function M:ClearStatusStrip(frame)
    if frame._statusStrip then
        frame._statusStrip:Hide()
    end
end
...
function M:CreateHeader(parent, text, level)
    level = level or 2
    local size = (level == 1) and self.SIZE_H1 or (level == 2) and self.SIZE_H2 or self.SIZE_H3
...
function M:CreateStatus(parent, text, status)
    ...
    if status == "ok" then
        clr = self.CLR_TEXT_SUCCESS
    end
    if status == "warning" then
        clr = self.CLR_TEXT_WARNING
    end
    if status == "danger" then
        clr = self.CLR_TEXT_DANGER
    end
