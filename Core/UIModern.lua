...
function M:InitDrawer()
    ...
    -- The drawer only exists where the Guide tab does: a client whose profile
    -- includes it AND that has guide content with steps (Retail today). TBC
    -- Anniversary has no Guide tab; Mists has one but no guides yet.
    if TA.IsTabAvailable and not TA:IsTabAvailable("guide") then
        self._guideDisabled = true
        return nil
    end
    ...
    mainFrame:HookScript("OnShow", function()
        if isOpen and not TA._drawerSuppressed and not M._guideDisabled then
            drawer:Show()
...
--- Hides the drawer and its edge toggle when the Guide tab is switched off in
--- options (or doesn't exist on this client); restores them when it's back on.
function M:UpdateDrawerAvailability()
    local enabled = (not TA.IsTabEnabled) or TA:IsTabEnabled("guide")
    self._guideDisabled = not enabled
    if not self.drawer then return end
    if enabled then
        if self.drawerToggle then self.drawerToggle:Show() end
        local open = not (TA.db and TA.db.drawerOpen == false)
        if open and TA.UI and TA.UI:IsVisible() and not TA._drawerSuppressed then
            self.drawer:Show()
            self.drawer:SetWidth(self.DRAWER_WIDTH)
            self.drawer:SetAlpha(1)
        end
    else
        self.drawer:Hide()
        if self.drawerToggle then self.drawerToggle:Hide() end
    end
end
