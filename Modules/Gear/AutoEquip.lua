-- ToonAge/Modules/Gear/AutoEquip.lua
...
local function ItemIsWearableByClass(itemLink, equipLoc)
    ...
    local jewellery = {
        ...
        INVTYPE_NECK = 1,
        INVTYPE_CLOAK = 1,
        INVTYPE_HEAD = 1, -- helms are class-agnostic for ilvl check
    }
    if jewellery[equipLoc] then
        return true
    end
    ...
    if not allowed then
        return true
    end -- unknown class: allow

    local _, _, _, _, _, _, itemSubType = GetItemInfo(itemLink)
    if not itemSubType then
        return true
    end -- cache miss: allow (will re-check on BAG_UPDATE_DELAYED)

    for _, a in ipairs(allowed) do
        if itemSubType:find(a) then
            return true
        end
    end
    return false
...
local function BestSlotForItem(itemLink, equipLoc, newIlvl)
    ...
    if equipLoc == "INVTYPE_2HWEAPON" then
        ...
        if ohLink then
            return nil
        end -- has off-hand: skip auto-equip
    end
...
local function EvaluateItem(itemLink)
    ...
    -- Armour-type veto
    if not ItemIsWearableByClass(itemLink, equipLoc) then
        return
    end
    ...
    TA:Raw(
        TA.LOG.OUTPUT,
        string.format(
            "|cFFFFD100[TA]|r Auto-equipped |cFF1EFF00%s|r (ilvl %d → slot %d).",
            itemName,
            ilvl,
            targetSlot
        )
