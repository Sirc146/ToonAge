-- ToonAge/Modules/TooltipScorer.lua
...
local function CompareDualSlot(itemLink, slots)
    ...
    if newScore <= 0 then return 0, 0, 0, false end
...
function TooltipScorer:EvaluateItem(tooltip, itemLink)
    ...
    if newScore <= 0 and not isEmpty then return end
...
function TooltipScorer:AppendLine(tooltip, pct, newScore, eqScore, isEmpty)
    ...
    if isEmpty then
        ...
        local text = string.format(
            "%s%s New%s  %s(%d)%s",
            COLOR_GREEN, ARROW_UP, COLOR_END,
            COLOR_CYAN, math.floor(newScore), COLOR_END
        )
    ...
    elseif pct > threshold then
        ...
        local text = string.format(
            ...
            COLOR_GREEN, ARROW_UP, pct, COLOR_END,
            math.floor(newScore), math.floor(eqScore)
        )
        tooltip:AddLine(text)
    elseif pct < -threshold then
        -- Downgrade
        local text = string.format(
            ...
            COLOR_RED, ARROW_DOWN, pct, COLOR_END,
            math.floor(newScore), math.floor(eqScore)
        )
        tooltip:AddLine(text)
    elseif math.abs(pct) <= threshold then
        ...
        local text = string.format(
            ...
            COLOR_YELLOW, ARROW_SIDE, pct, COLOR_END,
            math.floor(newScore), math.floor(eqScore)
        )
        tooltip:AddLine(text)
    end
       ...
       and Enum and Enum.TooltipDataType and Enum.TooltipDataType.Item then
        ...
        local ok, err = pcall(function()
            TooltipDataProcessor.AddTooltipPostCall(Enum.TooltipDataType.Item, function(tooltip, data)
                if not data or not data.hyperlink then return end
