...
-- Midnight Season 2 upgrade currencies (Mistcrests). These replaced The War
-- Within's Harbinger Crests, which this panel still listed until 2026-09-17.
-- IDs: Adventurer 3442 (Wowhead, Warcraft Wiki); Veteran-Myth 3443-3446
-- (conquestcapped.com Season 2 crest guide). /ta spellaudit checks every ID
-- and name against the live client.
Gear.UPGRADE_CURRENCIES = {
    { id = 3442, name = "Adventurer Mistcrest", color = "|cFF9D9D9D" },
    { id = 3443, name = "Veteran Mistcrest",    color = "|cFF1EFF00" },
    { id = 3444, name = "Champion Mistcrest",   color = "|cFF0070DD" },
    { id = 3445, name = "Hero Mistcrest",       color = "|cFFA335EE" },
    { id = 3446, name = "Myth Mistcrest",       color = "|cFFFF8000" },
}
...
local CORE_GRID_SLOTS = {
    { slotID = 1,  name = "Head",      isEnchantable = true },
    { slotID = 2,  name = "Neck" },
    { slotID = 3,  name = "Shoulder",  isEnchantable = true },
    { slotID = 15, name = "Back" },
...
-- ── Item scoring ──────────────────────────────────────────────────────
-- Which equipped item a candidate would replace, for the DR baseline below.
-- Rings and trinkets: the copy of this item if it's worn, else the weaker slot.
local SCORE_SLOT = {
    ...
    INVTYPE_WEAPONMAINHAND = { 16 }, INVTYPE_RANGED = { 16 }, INVTYPE_RANGEDRIGHT = { 16 },
    INVTYPE_SHIELD = { 17 }, INVTYPE_WEAPONOFFHAND = { 17 }, INVTYPE_HOLDABLE = { 17 },
}

--- Stats of the equipped item this one would replace. Subtracted from the
--- player's live ratings before DR is applied, so the candidate and the item
--- it replaces are measured from the SAME starting point. Without this the
--- equipped item's own stats were counted twice (once in the live rating,
--- once as "added"), pushing it deeper into DR than any alternative.
local function ReplacedItemStats(itemLink)
    local equipLoc = select(4, GetItemInfoInstant(itemLink))
    local slots = equipLoc and SCORE_SLOT[equipLoc]
    if not slots then return nil end
    local itemID = GetItemInfoInstant(itemLink)
    local chosen
    for _, s in ipairs(slots) do
        local link = GetInventoryItemLink("player", s)
        if link and GetItemInfoInstant(link) == itemID then chosen = link; break end
    end
    if not chosen then
        local weakest, weakestSum
        for _, s in ipairs(slots) do
            local link = GetInventoryItemLink("player", s)
            if not link then weakest, weakestSum = nil, -1; break end
            local st = U.ReadItemStats(link)
            local sum = st.CRIT + st.HASTE + st.MASTERY + st.VERS
            if not weakestSum or sum < weakestSum then weakest, weakestSum = link, sum end
        end
        chosen = weakest
    end
    return chosen and U.ReadItemStats(chosen) or nil
end

local function CalculateItemScore(itemLink, specID, mode)
    if not itemLink then return 0 end
    local stats = U.ReadItemStats(itemLink)
    ...
    if TA.StatEngine and mode ~= "pvp" and specID == playerSpecID then
        baseScore = CleanProxyValue(TA.StatEngine:ScoreItem(stats, specID, nil, ReplacedItemStats(itemLink)))
    else
...
local function ScanBagsForUpgrades(slotID, specID, mode, currentScore, itemDef, claimedBags, twoHandBaseline)
    ...
    for _, item in ipairs(Gear.bagCache.items) do
        ...
        if equipLoc then
            ...
            if isValid then
                ...
                -- Fix (2026-09-13): a two-handed candidate replaces BOTH Main
                -- Hand and Off Hand, so it only counts as a real upgrade if it
                -- beats what's currently equipped in both hands COMBINED --
                -- not just the Main Hand score alone (all `bestScore`
                -- reflects here). Without this, a 2H weapon could out-score a
                -- dual-wielder's Main Hand by itself while still being an
                -- overall downgrade once the Off Hand's stats are lost.
                local meetsUpgradeBar = score > bestScore
                if twoHandBaseline and equipLoc == "INVTYPE_2HWEAPON" then
                    meetsUpgradeBar = meetsUpgradeBar and score > twoHandBaseline
                end

                if meetsUpgradeBar then
...
-- ── Enchantable slots (Midnight) ───────────────────────────────────────
-- Midnight moved enchants back to Head/Shoulder and dropped Back/Wrist:
-- Head, Shoulder, Chest, Legs (spellthread / armor kit), Feet, both Rings and
-- weapons (method.gg "List of all Midnight Consumables, Enchants and Gems",
-- 2026-09-06). The grid previously used The War Within's set, so every cloak
-- and bracer showed "Missing Enchant" while helms and shoulders were never
-- checked. Off Hand only counts when it is actually a weapon.
local ENCHANTABLE_SLOT = { [1]=true, [3]=true, [5]=true, [7]=true, [8]=true,
                           [11]=true, [12]=true, [16]=true, [17]=true }
local OFFHAND_WEAPON_LOC = { INVTYPE_WEAPON=true, INVTYPE_WEAPONOFFHAND=true,
                             INVTYPE_WEAPONMAINHAND=true, INVTYPE_2HWEAPON=true }
local function IsEnchantableSlot(slotID, itemLink)
    if not ENCHANTABLE_SLOT[slotID] then return false end
    if slotID == 17 and itemLink then
        local getInstant = (C_Item and C_Item.GetItemInfoInstant) or GetItemInfoInstant
        local ok, _, _, _, equipLoc = pcall(getInstant, itemLink)
        if ok and equipLoc and not OFFHAND_WEAPON_LOC[equipLoc] then return false end
    end
    return true
end
...
local function AuditItemEnchants(itemLink, isEnchantable)
    ...
    if enchantID == 0 then
        -- Don't nag about enchants that aren't worth the gold. While leveling,
        -- gear is replaced every few levels; at max level, anything below the
        -- current season's Champion track is transitional. Only a max-level
        -- item at Champion ilvl or above is flagged, and a missing weapon
        -- enchant is called out as the costliest gap.
        local level    = UnitLevel("player") or 1
        local maxLevel = (GetMaxLevelForPlayerExpansion and GetMaxLevelForPlayerExpansion())
                      or (GetMaxPlayerLevel and GetMaxPlayerLevel()) or level
        if level < maxLevel then
            return true, "|cFF888780No enchant (skip while leveling)|r"
        end
        local Z = TA.Data and TA.Data.Zones
        local floor = Z and Z.TRACKS and Z.TRACKS.champion and Z.TRACKS.champion.ilvlMin
        local getIlvl = (C_Item and C_Item.GetDetailedItemLevelInfo) or GetDetailedItemLevelInfo
        local okI, ilvl = pcall(getIlvl, itemLink)
        if floor and okI and type(ilvl) == "number" and ilvl < floor then
            return true, "|cFF888780No enchant (optional below ilvl " .. floor .. ")|r"
        end
        local getInstant = (C_Item and C_Item.GetItemInfoInstant) or GetItemInfoInstant
        local okL, _, _, _, equipLoc = pcall(getInstant, itemLink)
        if okL and equipLoc and equipLoc:find("WEAPON") then
            return false, "|cFFFF4444Missing Weapon Enchant|r"
        end
        return false, "|cFFFF4444Missing Enchant|r"
    end
...
function Gear:RenderPlayerGrid(content, sidebar, padL, y, w, pvxMode)
    ...
    -- Fetched up front, independent of the grid loop's Main-Hand-before-
    -- Off-Hand ordering, so it's already known when Main Hand itself is
    -- being scanned below (a two-handed candidate for Main Hand replaces
    -- BOTH hands, so its true baseline is Main Hand + Off Hand combined,
    -- not Main Hand alone -- see twoHandBaseline below).
    local prefetchOHLink  = GetInventoryItemLink("player", 17)
    local prefetchOHScore = CalculateItemScore(prefetchOHLink, specID, pvxMode)
    ...
    for _, slotDef in ipairs(CORE_GRID_SLOTS) do
        ...
        if slotDef.slotID == 16 then
            ...
            -- A 2H candidate must beat Main Hand + Off Hand combined (both
            -- get replaced) -- unless Main Hand is already 2H, in which case
            -- Off Hand is already empty/unusable and there's nothing extra
            -- to lose.
            twoHandBaseline = currentScore + (twoHandEquipped and 0 or prefetchOHScore)
        end

        local bestBagLink, bestBagScore, bBag, bSlot, claimedBy, claimedLink, claimedBag, claimedSlot =
            ScanBagsForUpgrades(slotDef.slotID, specID, pvxMode, currentScore, slotDef, claimedBags, twoHandBaseline)
        ...
        elseif currentLink then
            if IsEnchantableSlot(slotDef.slotID, currentLink) then
    ...
    local currencies = Gear.UPGRADE_CURRENCIES

    local cx = padL
    for _, c in ipairs(currencies) do
        local info  = C_CurrencyInfo and C_CurrencyInfo.GetCurrencyInfo and C_CurrencyInfo.GetCurrencyInfo(c.id)
        local count = info and info.quantity or 0
        -- Mistcrest caps accrue weekly: show the current season maximum when the
        -- client reports one, otherwise just the count.
        local max   = info and info.maxQuantity and info.maxQuantity > 0 and info.maxQuantity or nil
        ...
        cVal:SetText(c.color .. count .. "|r" .. (max and (" / " .. max) or ""))
...
function Gear:RenderTargetGrid(content, sidebar, padL, y, w)
    ...
    for _, slotDef in ipairs(CORE_GRID_SLOTS) do
        ...
        local isEnchanted, auditStr = AuditItemEnchants(currentLink, IsEnchantableSlot(slotDef.slotID, currentLink))
...
function Gear:RenderSidebarDetails(parent, slotID, curLink, upLink, curScore, upScore, isTarget,
    ...
    if curLink then
        y = y - 4
        local isOk, auditStr = AuditItemEnchants(curLink, IsEnchantableSlot(slotID, curLink))
...
local function InjectTooltipScore(tooltip, itemLink)
    ...
    -- Fix (2026-09-13): weapon slots need hand-aware comparisons, not a flat
    -- "what's in this slot right now":
    --   * A two-handed candidate replaces BOTH hands, so it must beat Main
    --     Hand + Off Hand combined, not Main Hand alone -- otherwise a 2H
    --     could show as a big upgrade over just the Main Hand while actually
    --     being a net loss once the Off Hand's stats are given up too.
    --   * An off-hand-only item (shield/holdable/off-hand weapon) can't
    --     physically be equipped while a two-handed weapon occupies Main
    --     Hand -- doing so unequips the 2H entirely. GetInventoryItemLink for
    --     Off Hand returns nothing in that case, so equippedScore was 0 and
    --     the diff below always showed a false "upgrade" for any off-hand
    --     item hovered while wielding a 2H (the "dual wield" bug report).
    --     Compare against the 2H's own score instead, since that's what
    --     equipping this item would actually cost.
    if equipLoc == "INVTYPE_2HWEAPON" or equipLoc == "INVTYPE_WEAPONOFFHAND"
        or equipLoc == "INVTYPE_HOLDABLE" or equipLoc == "INVTYPE_SHIELD" then
        local mhLink = GetInventoryItemLink("player", 16)
        local _, _, _, _, _, _, _, _, mhEquipLoc = mhLink and GetItemInfo(mhLink)
        local mhTwoHand = (mhEquipLoc == "INVTYPE_2HWEAPON")
        local mhScore = CalculateItemScore(mhLink, specID, pvxMode)

        if equipLoc == "INVTYPE_2HWEAPON" then
            local ohLink = GetInventoryItemLink("player", 17)
            local ohScore = (not mhTwoHand) and CalculateItemScore(ohLink, specID, pvxMode) or 0
            equippedScore = mhScore + ohScore
        elseif mhTwoHand then
            -- Off-hand-only item, but Main Hand is already a 2H: equipping
            -- this sacrifices the 2H's full value, not a free addition.
            equippedScore = mhScore
        end
    end
