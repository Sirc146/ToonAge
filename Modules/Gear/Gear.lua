...
local function ResolveArmorType()
    ...
    if cls == "WARRIOR" or cls == "PALADIN" or cls == "DEATHKNIGHT" then
        return 4 -- Plate
    elseif cls == "HUNTER" or cls == "SHAMAN" or cls == "EVOKER" then
        return 3 -- Mail
...
local function GetItemIlvls(itemLink)
    ...
    if pvpIlvl then
        Gear.pvpIlvlCache[itemLink] = pvpIlvl
    end
...
local function CalculateItemScore(itemLink, specID, mode)
    ...
    local total = stats.INT + stats.AGI + stats.STR + stats.STAM + stats.CRIT + stats.HASTE + stats.MASTERY + stats.VERS
    ...
    if src == "pawn" and Pawn and Pawn.GetSingleItemValue then
        local ok, val = pcall(function()
            return Pawn.GetSingleItemValue(itemLink, false)
        end)
        if ok and val then
            return CleanProxyValue(val)
        end
    end
...
local function ScanBagsForUpgrades(slotID, specID, mode, currentScore, itemDef, claimedBags)
    ...
    for _, item in ipairs(Gear.bagCache.items) do
        ...
        if equipLoc then
            local isValid = false
            if slotID == 1 and equipLoc == "INVTYPE_HEAD" then
                isValid = true
            elseif slotID == 2 and equipLoc == "INVTYPE_NECK" then
                isValid = true
            elseif slotID == 3 and equipLoc == "INVTYPE_SHOULDER" then
                isValid = true
            elseif slotID == 15 and equipLoc == "INVTYPE_CLOAK" then
                isValid = true
            elseif slotID == 5 and (equipLoc == "INVTYPE_CHEST" or equipLoc == "INVTYPE_ROBE") then
                isValid = true
            elseif slotID == 9 and equipLoc == "INVTYPE_WRIST" then
                isValid = true
            elseif slotID == 10 and equipLoc == "INVTYPE_HAND" then
                isValid = true
            elseif slotID == 6 and equipLoc == "INVTYPE_WAIST" then
                isValid = true
            elseif slotID == 7 and equipLoc == "INVTYPE_LEGS" then
                isValid = true
            elseif slotID == 8 and equipLoc == "INVTYPE_FEET" then
                isValid = true
            elseif itemDef.isRing and equipLoc == "INVTYPE_FINGER" then
                isValid = true
            elseif itemDef.isTrinket and equipLoc == "INVTYPE_TRINKET" then
                isValid = true
            elseif
                slotID == 16
                and (
            ...
            if isValid and itemClassID == 4 then
                local isAccessory = equipLoc == "INVTYPE_NECK"
                    or equipLoc == "INVTYPE_CLOAK"
                    or equipLoc == "INVTYPE_FINGER"
                    or equipLoc == "INVTYPE_TRINKET"
                    or equipLoc == "INVTYPE_SHIELD"
                    or equipLoc == "INVTYPE_HOLDABLE"
                if not isAccessory and itemSubClassID ~= PRIMARY_ARMOR_TYPE then
                    isValid = false
                end
            end
                ...
                if score > bestScore then
                    if claimedBags[bagSlotKey] then
                        if score > bestClaimedScore then
                            ...
                            claimedLink = item.link
                            claimedBag = item.bag
                            claimedSlot = item.slot
...
local function RebuildProfessionsCache()
    ...
    for _, idx in ipairs({ p1, p2, arch, fish, cook, fa }) do
        if idx then
            ...
            if name then
                PlayerProfessionsCache[name:upper()] = true
            end
        end
    end
end
...
local function AuditItemEnchants(itemLink, isEnchantable)
    if not itemLink or not isEnchantable then
        return true, ""
    end
...
function Gear:Render(content, sidebar)
    ...
    for setName, setData in pairs(gearSets) do
        ...
        if setData.slots then
            for _ in pairs(setData.slots) do
    ...
    hdr:SetText(
        (self.viewMode == "player" and "GEAR ADVISOR: BAG SCANNER" or "GEAR ADVISOR: TARGET SCOUTER")
            .. " |cFF888780(Weights: "
            .. weightSource:upper()
            .. ")|r"
    )
    ...
    srcBtn:SetScript("OnClick", function()
        local next = weightSource == "built-in" and "pawn" or weightSource == "pawn" and "custom" or "built-in"
        if TA.charDB then
            TA.charDB.weightSource = next
        end
        self:Render(content, sidebar)
    end)
    ...
    disc:SetText(
        "|cFFAAAAAAPvP gear shows world ilvl + PvP ilvl (green) when available. Click any slot to see score details.|r"
    )
...
function Gear:RenderPlayerGrid(content, sidebar, padL, y, w, pvxMode)
    ...
    for _, slotDef in ipairs(CORE_GRID_SLOTS) do
        ...
        if slotDef.isRing then
            targetSlotID = (slotDef.ringIdx == 1 and 11 or 12)
        end
        if slotDef.isTrinket then
            targetSlotID = (slotDef.triIdx == 1 and 13 or 14)
        end
        ...
        elseif claimedBy then
            ...
            if M then
                M:ApplyStatusStrip(row, "warning")
            end
        ...
        row:SetScript("OnEnter", function(s)
            ...
            if currentLink then
                ...
                if capHasUp then
                    GameTooltip:AddLine(
                        "Bag upgrade: +" .. math.floor(capBestScore - capScore) .. " score",
                        0.12,
                        1.0,
                        0.0
                    )
                ...
                if capHeirloomCap then
                    if capHeirloomWarn then
                        GameTooltip:AddLine(
                            "[!] Heirloom scales to level " .. capHeirloomCap .. " — upgrade soon",
                            1.0,
                            0.53,
                            0.0
                        )
...
function Gear:RenderTargetGrid(content, sidebar, padL, y, w)
    ...
    statText:SetText(
        string.format(
            "Inspecting: |cFFFFFFFF%s|r  |cFF888780(Level %d %s)|r",
            tName,
            tLevel,
            tClass:lower():gsub("^%l", string.upper)
        )
    )
...
-- ── Sidebar details ───────────────────────────────────────────────────
function Gear:RenderSidebarDetails(
    parent,
    slotID,
    curLink,
    upLink,
    ...
    if isTarget then
        ...
        SLabel(
            curLink and (GetItemInfo(curLink) or "Equipped Item") or "Empty Slot",
            10,
            curLink and 1 or 0.5,
            curLink and 1 or 0.5,
            curLink and 1 or 0.5
        )
...
local function InjectTooltipScore(tooltip, itemLink)
    ...
    if tooltip[TOOLTIP_INJECTED_KEY] == itemLink then
        return
    end
...
function Gear:Init()
    ...
    if TooltipDataProcessor and TooltipDataProcessor.AddTooltipPostCall then
        ...
        hooksecurefunc(GameTooltip, "SetBagItem", function(tt, bag, slot)
            ...
            if itemLink then
                InjectTooltipScore(tt, itemLink)
            end
        end)
