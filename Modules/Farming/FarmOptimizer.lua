-- ToonAge/Modules/Farming/FarmOptimizer.lua
    ...
    for i = 1, numItems do
        ...
        if ok and link then
            ...
            if ok2 and classID == 7 then
                if subclassID == GATHER_SUBCLASS.HERB then
                    return "herb"
                end
                if subclassID == GATHER_SUBCLASS.ORE then
                    return "ore"
                end
                if subclassID == GATHER_SUBCLASS.LEATHER then
                    return "skin"
                end
            end
        end
    ...
    if lastPos then
        ...
        if yards < 0.5 then -- less than 0.5 yards in interval = stationary
            ...
            elseif (now - idleStart) > SCORING.IDLE_THRESHOLD then
                FarmOpt.session.totalIdleTime = FarmOpt.session.totalIdleTime + SCORING.POSITION_INTERVAL
            end
        else
            idleStart = nil
        end
...
function FarmOpt:GetAvgDistancePerNode()
    if not self.session or #self.session.nodes < 1 then
        return 0
    end
...
function FarmOpt:GetEfficiencyGrade()
    ...
    if npm >= SCORING.GRADE_A then
        return "A"
    end
    if npm >= SCORING.GRADE_B then
        return "B"
    end
    if npm >= SCORING.GRADE_C then
        return "C"
    end
    if npm >= SCORING.GRADE_D then
        return "D"
    end
    return "F"
end
...
function FarmOpt:GetZoneHeat()
    ...
    if count >= SCORING.HEAT_HOT then
        return "Hot"
    end
    if count >= SCORING.HEAT_WARM then
        return "Warm"
    end
    return "Cold"
end
...
function FarmOpt:GetPrimaryBehavior()
    ...
    if h / total >= 0.70 then
        return "herb"
    end
    if o / total >= 0.70 then
        return "ore"
    end
    if s / total >= 0.70 then
        return "skin"
    end
...
function FarmOpt:GetSuggestion()
    local now = GetTime()
    if (now - lastSuggestionTime) < SUGGESTION_COOLDOWN then
        return nil
    end
    ...
    if #clusters > 0 and lastPos then
        ...
        for _, c in ipairs(clusters) do
            if c.strength > best.strength then
                best = c
            end
        end
...
local function PrintSessionSummary()
    ...
    local gradeHex = string.format("|cff%02x%02x%02x", gradeColor[1] * 255, gradeColor[2] * 255, gradeColor[3] * 255)

    TA:Raw(TA.LOG.OUTPUT, "|cff00ccff[ToonAge FarmOpt]|r Session Summary:")
    TA:Raw(
        TA.LOG.OUTPUT,
        string.format("  Duration: %dm %ds | Nodes: %d | Type: %s", minutes, seconds, #s.nodes, behavior)
    )
    TA:Raw(TA.LOG.OUTPUT, string.format("  Nodes/min: %.1f | Gold/hr: %.0fg", npm, gph))
    TA:Raw(TA.LOG.OUTPUT, string.format("  Grade: %s%s|r | Score: %d/100 | Heat: %s", gradeHex, grade, score, heat))
    TA:Raw(
        TA.LOG.OUTPUT,
        string.format(
            "  Distance: %.0f yds | Idle: %.0fs | Combat: %.0fs",
            s.totalDistance,
            s.totalIdleTime,
            s.totalCombatTime
        )
