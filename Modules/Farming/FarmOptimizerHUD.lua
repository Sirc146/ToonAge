-- ToonAge/Modules/Farming/FarmOptimizerHUD.lua
...
local function CheckGatheringProfession()
    ...
    for i = 1, 2 do
        ...
        if idx then
            ...
            if GATHERING_PROFS[skillLine] then
                return true
            end
        end
    end
...
local function UpdateHUD()
    ...
    if not session or (session.nodeCount or 0) == 0 then
        if hudPanel:IsShown() and (GetTime() - lastActivity) > AUTO_HIDE_DELAY then
            FadeOut(hudPanel, FADE_DURATION, function()
                hudPanel:Hide()
            end)
            if suggestionBar and suggestionBar:IsShown() then
                FadeOut(suggestionBar, FADE_DURATION, function()
                    suggestionBar:Hide()
                end)
            end
        end
    ...
    hudPanel.nodesPerMin:SetText(
        string.format("|cFF%02x%02x%02x%.1f|r /min", gc[1] * 255, gc[2] * 255, gc[3] * 255, npm)
    )

    -- Gold/hour
    local gph = FarmOpt.GetGoldPerHour and FarmOpt:GetGoldPerHour() or 0
    hudPanel.goldPerHour:SetText(
        string.format(
            "Gold/hr: |cFFFFD100%s|r",
            U.FormatGold and U:FormatGold(gph) or string.format("%.0fg", gph / 10000)
        )
    )

    -- Grade
    hudPanel.grade:SetText(string.format("Route: |cFF%02x%02x%02x%s|r", gc[1] * 255, gc[2] * 255, gc[3] * 255, grade))

    -- Heat
    local heat = FarmOpt.GetHeat and FarmOpt:GetHeat() or "Cold"
    local hc = HeatColor(heat)
    hudPanel.heat:SetText(string.format("Zone: |cFF%02x%02x%02x%s|r", hc[1] * 255, hc[2] * 255, hc[3] * 255, heat))
...
function FOHUD:UpdateSummary()
    ...
    table.insert(
        lines,
        string.format(
            "Time: %s (active %s, idle %s, combat %s)",
            FormatTime(elapsed),
            FormatTime(active),
            FormatTime(idle),
            FormatTime(combat)
