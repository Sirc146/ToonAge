...
local function ActiveWeaponSkill(ranged)
    local Scan = TA.SkillScan
    if not Scan then return U.GetPlayerLevel() * 5, false, "assumed maximum" end

    local skill, measured = Scan:GetActiveWeaponSkill(ranged)
    if measured then
        return skill, true, ranged and "from your equipped ranged weapon" or "from your equipped main hand"
    end
...
local function ShortfallText(ratingKey, pctNeeded)
    -- ratingKey HIT_MELEE / HIT_RANGED / HIT_SPELL
...
function M:Collect()
    ...
    local skill, skillMeasured, skillLabel = ActiveWeaponSkill(ranged)
    local dualWield = (not ranged) and IsDualWielding() or false
    ...
    if physical then
        out.caps.meleeHit = S:GetMeleeHitCap({
            contextKey = ctxKey, weaponSkill = skill, ranged = ranged,
            dualWield = dualWield, bonusHitPercent = meleeBonus,
        })
        -- Expertise only reduces dodge/parry on melee attacks; it does nothing
        -- for a hunter's ranged shots, so no target is shown for ranged.
        if not ranged then
...
local function RenderContextPicker(content, y, activeKey)
    ...
    if TA.db and TA.db.pvpMode then
        ...
        y = L:ButtonRow(content, y, {
            { label = "Switch to PvE", onClick = function() TA.db.pvpMode = false; L:RefreshUI() end },
        })
        return L:Spacer(y, 6)
    end
    ...
    local auto = (not TA.db) or (TA.db.capContext == nil) or (TA.db.capContext == "auto")
    local level = U.GetPlayerLevel()
    local active = S:GetContext(activeKey)
    local tLevel = level + active.delta
    y = L:Paragraph(content, y, string.format(
        "You are level |cFFFFD100%d|r. Caps below are against a |cFFFFD100level %d|r target "
        .. "(defense %d) — %s.", level, tLevel, S:TargetDefense(tLevel),
        auto and "following your level automatically" or "pinned by you"))
    if not auto then
        y = L:ButtonRow(content, y, {
            { label = "Follow my level", onClick = function() TA.db.capContext = "auto"; L:RefreshUI() end,
              tooltip = { "Follow my level", "Same-level mobs while leveling, raid bosses at 70." } },
        })
    end
...
function M:Render(content, side)
    ...
    if data.weaponSkill < maxSkill then
        y = L:Paragraph(content, y, string.format(
            "|cFFFF9A1AYour " .. (data.role == "RANGED" and "ranged " or "") .. "weapon skill is %d of a possible %d.|r Each missing point adds "
    ...
    if data.noCaps then
        ...
        y = L:Paragraph(content, y, "Off-tanking or off-DPSing? Pick that role:", { color = L.C_SECONDARY })
        y = L:RoleButtons(content, y)
    ...
    if hit then
        ...
        if hit.capped then
            ...
            note = "Need " .. ShortfallText(hit.ranged and "HIT_RANGED" or "HIT_MELEE", hit.needed)
        end
    ...
    if def then
        ...
        if def.capped then
            ...
            note = string.format("Need %d more defense skill — you can still be critically hit "
                .. "%s of the time.",
                def.needed, U.Pct(math.max(def.critTaken - def.critRemoved
                    - (def.critFromTalents or 0) - (def.critFromResilience or 0), 0)))
        end
        ...
        y = L:CapBar(content, y, {
            ...
            tooltip = {
                ...
                string.format("Talents remove %s and resilience %s of that, so this cap is %d "
                    .. "instead of the bare %d (bears with 3/3 Survival of the Fittest: 415).",
                    U.Pct(def.critFromTalents or 0), U.Pct(def.critFromResilience or 0),
                    def.cap, def.baseDefense + math.ceil(def.critTaken / 0.04 - 1e-9)),
            },
        })
    end
    ...
    if data.hitSources and #data.hitSources > 0 then
        ...
        for _, src in ipairs(data.hitSources) do
            ...
            if src.override then
                text = "|cFFFFD100Manual override in force|r — talent and racial detection is "
                    .. "ignored. Use the button below to go back to automatic."
        ...
        if TA.charDB and (TA.charDB.hitBonusMelee or TA.charDB.hitBonusSpell) then
            y = L:ButtonRow(content, y, {
                { label = "Use automatic hit detection", onClick = function()
    ...
    y = L:Bullet(content, y,
        "Caps are computed from your level, weapon skill and target level — not read from a "
        .. "level-70 table.", { color = L.C_DIM })
    y = L:ButtonRow(content, y, {
        { label = "Print API checks to chat", onClick = function() M.SlashCommands.dumpme(M) end,
          tooltip = { "API checks", "Prints /dump lines that confirm the values this tab reads. Paste the results back if something looks wrong." } },
        { label = "Print caps to chat", onClick = function() M.SlashCommands.caps(M) end },
    })
...
M.SlashCommands = {
    ...
    context = function(self, args)
        local key = (args or ""):lower()
        local valid = { auto = true, same = true, plus1 = true, plus2 = true, plus3 = true }
        if valid[key] then
            TA.db.capContext = key
            if TA.UI and TA.UI:IsVisible() then TA.UI:Refresh() end
        else
            TA:Raw(TA.LOG.OUTPUT, "Usage: /ta context <auto|same|plus1|plus2|plus3>")
        end
    end,

    pvp = function(self)
        TA.db.pvpMode = not TA.db.pvpMode
        TA:Printf(TA.LOG.OUTPUT, nil, "PvP mode %s.", TA.db.pvpMode and "ON" or "OFF")
        if TA.UI and TA.UI:IsVisible() then TA.UI:Refresh() end
    end,
