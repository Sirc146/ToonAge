...
--- Single-number score for AutoEquip (Modules/TBC/AutoEquip.lua calls
--- GearMod.CalculateItemScore(link) with a plain function call). Without this,
--- AutoEquip silently fell back to raw item level on TBC and could swap a
--- warrior into higher-ilvl spell plate. Uses the same cap-aware scoring the
--- Gear tab shows, against the player's current cap gaps.
function M.CalculateItemScore(itemLink)
    if not itemLink then return 0 end
    local role = U.InferRole()
    local okG, gaps = pcall(M.GetCapGaps, M)
    if not okG or type(gaps) ~= "table" then gaps = {} end
    local okS, score = pcall(M.ScoreItem, M, itemLink, role, gaps)
    if okS and type(score) == "number" then return score end
    return 0
end
            ...
            if link then
                ...
                if targetSlots and U.SafeNum(minLevel) <= U.GetPlayerLevel() then
                    ...
                    if equipLoc == "INVTYPE_2HWEAPON" then
                        ...
                        -- Fixed 2026-09-12: the offHandOnly check above only ever looked
                        -- at INVTYPE_WEAPONOFFHAND/HOLDABLE/SHIELD — the three equip
                        -- types that can ONLY go in slot 17. A plain one-handed weapon
                        -- (INVTYPE_WEAPON: dagger, sword, mace, axe, fist weapon) maps to
                        -- BOTH slots {16, 17}, so when the off-hand happened to be the
                        -- weaker of the pair it landed here as a bare, unblocked
                        -- "upgrade" — the exact same "off-hand recommended with no
                        -- main-hand to match" bug the 09-09 fix above addressed, just
                        -- through the one equip type that fix didn't cover. Applying the
                        -- identical main-hand check whenever a one-hand weapon's upgrade
                        -- actually resolves to the off-hand slot (never when it resolves
                        -- to slot 16 — replacing an empty or worse main-hand is always
                        -- fine) closes the gap without touching the already-correct
                        -- WEAPONOFFHAND/HOLDABLE/SHIELD path above.
                        if equipLoc == "INVTYPE_WEAPON" and worstSlot == 17 then
                            local mh = equipped[16]
                            if mh and mh.equipLoc == "INVTYPE_2HWEAPON" then
                                mainHandBlocks = true
                                mainHandNote = "can't actually be equipped yet — you're "
...
function M:Render(content, side)
    ...
    if unknownKeys > 0 then
        y = L:Paragraph(content, y, string.format(
            ...
            .. "scored. List them in chat and send them over to get them mapped.",
            unknownKeys), { color = L.C_WARNING })
        y = L:ButtonRow(content, y, {
            { label = "List unmapped stat keys", onClick = function() M.SlashCommands.statkeys() end },
        })
    end
