...
--- U.SafeGetNum returns a single number (0 on failure), never an ok flag.
--- Reading `local value, ok = U.SafeGetNum(...)` left ok nil on every call, so
--- Health, crit, haste, dodge, parry and block all showed "n/a". pcall directly.
...
local function RenderHeadline(content, y)
    ...
    if caps and caps.Collect then
        ...
        if ok and data then
            if data.noCaps then
                ...
                local order = {
                    { "meleeHit",  (role == "RANGED") and "ranged hit" or "melee hit", "pct" },
    ...
    local overridden = TA.charDB and TA.charDB.roleOverride and TA.charDB.roleOverride ~= "auto"
    y = L:Paragraph(content, y, string.format("Role: %s %s", role,
        overridden and "|cFF555049(set by you)|r" or "|cFF555049(read from class, talents and gear)|r"))
    y = L:RoleButtons(content, y)
    y = L:ButtonRow(content, y, {
        { label = "Open Stat Caps", onClick = function() L:OpenTab("caps") end,
          tooltip = { "Stat Caps", "Hit, expertise and defense targets in detail." } },
    })
    return L:Spacer(y, 6)
end
