...
function M:Render(content, side)
    ...
    local mech = TA.Data.GetRaceMechanics and TA.Data.GetRaceMechanics(token) or TA.Data.RaceMechanics[token]
    local y = -8
    ...
    if mech and (mech.meleeHitPercent or mech.spellHitPercent) then
        hasMechanical = true
        y = L:DataRow(content, y, {
            label = mech.spellHitPercent and "Spell hit" or "Melee/ranged hit", value = string.format("+%s", U.Pct(mech.spellHitPercent or mech.meleeHitPercent)),
