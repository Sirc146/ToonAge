...
--- Scan all bags for the best food the pet will actually eat. Preference:
--- food at or above the pet's level (cheapest such food first), then the
--- highest-level food still within 30 levels, then unknown-level food, then
--- conjured food; within a rank the smallest stack first, as Feed-O-Matic does.
--- @param dietSet table  set of diet strings the active pet accepts
--- @return table|nil  { bag, slot, name, icon, level, weak, count }
local function FindBestFood(dietSet)
    local F = TA.Data.PetFoods
    local petLevel = UnitLevel and UnitLevel("pet") or nil
    if petLevel and petLevel <= 0 then petLevel = nil end
    local best
    ...
    for bag = 0, 4 do
        ...
        for slot = 1, numSlots do
            ...
            if link then
                ...
                if entry and dietSet[entry.diet] and F:IsEdible(entry, petLevel) then
                    local count = SlotCount(bag, slot)
                    local r1, r2 = F:Rank(entry, petLevel)
                    if not best
                        or r1 < best.r1
                        or (r1 == best.r1 and r2 < best.r2)
                        or (r1 == best.r1 and r2 == best.r2 and count < best.count)
                    then
                        best = { bag = bag, slot = slot, name = name, icon = icon,
                                 level = entry.level, count = count, r1 = r1, r2 = r2,
                                 weak = (r1 == 2) }
                    end
                end
            end
...
function PetCare:Render(content, sidebar)
    ...
    if class == "HUNTER" then
        ...
        if food and not inCombat then
            ...
            label:SetText(("Feed  %s  (x%d)%s"):format(food.name, food.count,
                food.weak and "  |cFFFFAA33low level — little happiness|r" or ""))
