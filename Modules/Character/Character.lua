...
local DR_SOFT_CAP_L90 = {
    CRIT    = 1380,
    HASTE   = 1320,
    MASTERY = 1380,
    VERS    = 1620,
}
-- Level-aware: StatEngine scales the L90 bound by the live rating-per-% so a
-- leveling character's "DR soft cap" readout isn't pinned to max-level numbers.
local DR_SOFT_CAP = setmetatable({}, { __index = function(_, key)
    local SE = TA.StatEngine
    local v = SE and SE.GetFirstDRRating and SE:GetFirstDRRating(key)
    return (v and v > 0) and v or DR_SOFT_CAP_L90[key]
end })
