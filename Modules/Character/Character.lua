-- ToonAge/Modules/Character/Character.lua
...
function Character:OnEvent(event, ...)
    if not (TA.UI and TA.UI.activeTab == "character") then
        return
    end
    ...
    elseif event == "PLAYER_LEVEL_UP" or event == "UNIT_INVENTORY_CHANGED" or event == "PLAYER_EQUIPMENT_CHANGED" then
        self:UpdateData()
    end
end
...
local function GetVers()
    ...
    if ok and cr then
        return SafeCall(GetVersatilityBonus, SafeNum(cr))
    end
    return 0
end
...
function Character:RenderSidebar(sideChild)
    ...
    fade:SetHeight(180)
    fade:SetGradient("VERTICAL", CreateColor(0.04, 0.03, 0.01, 0), CreateColor(0.04, 0.03, 0.01, 0.96))
    ...
    edge:SetWidth(28)
    edge:SetGradient("HORIZONTAL", CreateColor(0, 0, 0, 0), CreateColor(0.04, 0.03, 0.01, 0.85))
    ...
    local groupStr = U.GetGroupType() == "solo" and "Solo" or U.GetGroupType() == "party" and "Party / M+" or "Raid"
    ...
    for i = 1, 4 do
        ...
        row:SetScript("OnEnter", function(btn)
            ...
            GameTooltip:AddDoubleLine(
                "Weight (" .. (Character.pvxMode == "pve" and "PvE" or "PvP") .. ")",
                string.format("%.2f", s.weight),
                0.7,
                0.7,
                0.7,
                rc[1],
            ...
            if s.pct >= 33.0 then
                local suggestion = s.redirectTarget
                        and ("Redirect itemization into " .. s.redirectTarget .. " instead.")
...
function Character:UpdateData()
    if not self.widgets.primName then
        return
    end
    ...
    local secondaries = {
        {
            ...
            rating = SafeCall(GetCombatRating, 1),
        },
