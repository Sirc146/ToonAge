-- ToonAge/Modules/Character/TalentsPvP.lua
...
local function ReadTreeNodes(configID, treeIDs)
    if not C_Traits or not treeIDs or #treeIDs == 0 then
        return nil
    end
                            ...
                            for _, entryID in ipairs(entryIDs) do
                                ...
                                if entryInfo then
                                    ...
                                    table.insert(entries, {
                                        ...
                                        name = (defInfo and defInfo.overrideName) or spellName or "Unknown",
                                        icon = (defInfo and defInfo.overrideIcon) or spellIcon or 134400,
                                    })
                                end
                            end
                        end
    ...
    for slot = 1, 3 do
        ...
        if ok and slotInfo and slotInfo.availableTalentIDs then
            for _, talentID in ipairs(slotInfo.availableTalentIDs) do
                local tok, info =
                    pcall(C_PvP.GetPvpTalentInfoByID or C_SpecializationInfo.GetPvpTalentInfoByID, talentID)
                if tok and info then
                    table.insert(talents, {
                        ...
                        icon = info.icon
                            or (C_Spell.GetSpellInfo(info.spellID) and C_Spell.GetSpellInfo(info.spellID).iconID)
                            or 134400,
                        slot = slot,
...
function TalentsPvP:Render(content, sidebar, startY)
    ...
    elseif self.treeCache then
        ...
        if nodes and #nodes > 0 then
            ...
            nodata:SetText(
                COL_GREY .. "No tree data available. Open your talent frame first to load tree info." .. CLOSE
            )
...
function TalentsPvP:RenderPvPTalentPanel(content, startY, w, padL, pvpData)
    ...
    hdr:SetText(
        COL_GOLD .. "PVP TALENT SLOTS" .. CLOSE .. "  " .. COL_GREY .. "(3 active in War Mode / Instanced PvP)" .. CLOSE
    )
    hdr:SetPoint("TOPLEFT", content, "TOPLEFT", padL, y)
    y = y - 18
    ...
    for slotIdx = 1, 3 do
        ...
        for _, talent in ipairs(slots[slotIdx]) do
            ...
            nameLbl:SetTextColor(
                isSelected and 0.29 or 0.55,
                isSelected and 1.0 or 0.50,
                isSelected and 0.48 or 0.40,
                1
            )
