-- ToonAge/Modules/Character/Pets.lua
...
local function RoleColor(role)
    if role == "FEROCITY" then
        return 1.00, 0.42, 0.29
    end
    if role == "TENACITY" then
        return 0.29, 0.65, 1.00
    end
    if role == "CUNNING" then
        return 1.00, 0.82, 0.29
    end
...
local function RoleLabel(role)
    ...
    if role == "TENACITY" then
        return "Tenacity  (Tank)"
    end
    if role == "CUNNING" then
        return "Cunning  (Utility)"
    end
...
local function RarityColor(rarity)
    if rarity == "Rare" then
        return 0.10, 0.44, 0.87
    end
    if rarity == "Rare Elite" then
        return 0.60, 0.10, 0.80
    end
...
local function CompRoleColor(role)
    ...
    if role == "HEALING" then
        return 0.29, 1.00, 0.50
    end
    if role == "CONTROL" then
        return 1.00, 0.82, 0.29
    end
    if role == "UTILITY" then
        return 0.80, 0.50, 1.00
    end
    if role == "SUPPORT" then
        return 0.50, 0.90, 1.00
    end
...
function Pets:RenderSidebar(sideChild)
    ...
    if hasPet then
        ...
        sideChild:HookScript("OnHide", function()
            model:Hide()
        end)
        ...
        if familyInfo then
            ...
            -- Ability tags
            local tags = {}
            if familyInfo.lust then
                table.insert(tags, "|cFFFF8800Bloodlust|r")
            end
            if familyInfo.mw then
                table.insert(tags, "|cFFFF4040Mortal Wounds|r")
            end
            if familyInfo.shield then
                table.insert(tags, "|cFF29A5FFShell Shield|r")
            end
            if familyInfo.exotic then
                table.insert(tags, "|cFFFF44FFExotic (BM)|r")
            end
        ...
        local msg = class == "HUNTER" and "No pet summoned.\nCall a pet to view its stats." or "No active companion."
...
function Pets:Render(content, sidebar)
    ...
    if sidebar then
        self.lastSidebar = sidebar
    end
    local sideRef = sidebar or self.lastSidebar
    if sideRef then
        self:RenderSidebar(sideRef)
    end
    ...
    if self.view == "active" then
        self:RenderActiveView(content, P, class, isHunter, isBM, w, padL, yBase)
    elseif self.view == "zone" then
        self:RenderZoneView(content, P, isBM, w, padL, yBase)
    elseif self.view == "stable" then
        self:RenderStableView(content, P, w, padL, yBase)
    elseif self.view == "finder" then
        self:RenderFinderView(content, P, isBM, w, padL, yBase)
    elseif self.view == "companions" then
        self:RenderCompanionView(content, P, class, w, padL, yBase)
    end
end
...
function Pets:RenderActiveView(content, P, class, isHunter, isBM, w, padL, y)
    ...
    if not hasPet then
        ...
        local msg = isHunter and "No pet summoned. Visit Zone Pets or Pet Finder to find a companion."
    ...
    if familyInfo then
        ...
        if familyInfo.exotic then
            table.insert(tags, "|cFFFF44FFExotic|r")
        end
        if #tags > 0 then
...
function Pets:RenderFinderView(content, P, isBM, w, padL, y)
    ...
    for _, entry in ipairs(sorted) do
        ...
        local tags = {}
        if info.lust then
            table.insert(tags, "|cFFFF8800Lust|r")
        end
        if info.mw then
            table.insert(tags, "|cFFFF4040MW|r")
        end
        if info.shield then
            table.insert(tags, "|cFF29A5FFShield|r")
        end
...
function Pets:RenderCompanionView(content, P, class, w, padL, y)
    ...
    for _, comp in ipairs(companions) do
        ...
        typeLbl:SetTextColor(
            ...
            comp.summonType == "Permanent" and 0.40 or 0.29,
            1
        )
