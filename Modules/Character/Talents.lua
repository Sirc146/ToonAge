...
function Talents:RenderContent(content, activeSpecID)
    ...
    local maxLevel    = (GetMaxLevelForPlayerExpansion and GetMaxLevelForPlayerExpansion()) or (GetMaxPlayerLevel and GetMaxPlayerLevel()) or 90
    ...
    if isMaxLevel then
        ...
        -- Re-verified for Midnight Season 2 "Curse of Ula'tek" (patch 12.1),
        -- 2026-09-05. The previous numbers here (dated 2026-08-31) were
        -- wrong — they were carried over from the same bad Delve-tier
        -- research pass that put Delves.lua's Tier 1-11 at 170-309 instead
        -- of the real 266-305; see Data/ItemLevels.lua's sourcing comment
        -- for the two-pass verification (icy-veins.com/wow/delves-guide
        -- quoted directly, cross-checked against a second independent
        -- pass) that replaced both. These four numbers now match
        -- Z.READINESS in Data/ItemLevels.lua — keep them in sync if that
        -- file changes again. LFR's 273 is Blizzard's own published
        -- "Raid Finder Minimum Item Level" from their raid-launch news post.
        local thresholds = {
            { label = "Delves    (Tier 8)",   req = 295 },
            { label = "LFR       (Raid)",     req = 273 },
            { label = "Normal    (Raid)",     req = 292 },
            { label = "Mythic+   (Base key)", req = 295 },
        }
        ...
        local mileTxt
        -- Midnight milestones (verified 2026-09-16): talent tree at 10; hero talents
        -- at 71, one point per level (nexttier.pro hero talent overview); the
    ...
    if specID == activeSpecID then
        ...
        local SPEC_WEAPON_NEEDS = {
            ...
            [1480] = nil,        -- Devourer: flexible (12.1 also allows daggers)
            -- Mage
                    ...
                    for slotID = 1, 17 do
                        ...
                        elseif (slotID == 16 or slotID == 17) and not weaponsIncluded then
                            ...
                            if link then
                                ...
                                if rawStats then
                                    local stats = U.ReadItemStats(link)
