-- ToonAge/Modules/Character/Talents.lua
...
function Talents:RenderContent(content, activeSpecID)
    ...
    local roleHex =
        string.format("%02x%02x%02x", math.floor(rdsp.r * 255), math.floor(rdsp.g * 255), math.floor(rdsp.b * 255))
    ...
    local customStr = TA.charDB
        and TA.charDB.customBuilds
    ...
    if build.nodes and #build.nodes > 0 then
        ...
        local pr, pg, pb
        if pct >= 80 then
            pr, pg, pb = 0.29, 1.00, 0.48
        elseif pct >= 50 then
            pr, pg, pb = 1.00, 0.82, 0.00
        else
            pr, pg, pb = 1.00, 0.27, 0.27
        end
    ...
    -- Header label
    local mHdrTxt = isMaxLevel and ("ENDGAME  ·  " .. math.floor(eqILvl) .. " iLvl equipped")
    ...
    if isMaxLevel then
        ...
        for i, row in ipairs(thresholds) do
            ...
            rdyLbl:SetText(
                ready and "|cFF4AFF7A\226\156\147 Ready|r" or ("|cFFFF5555\226\156\151 Need " .. row.req .. "|r")
            )
            ...
            for lvl = playerLevel, playerLevel + 20 do
                ...
                if t then
                    ...
                    if lvl == playerLevel then
                        pathLines[#pathLines + 1] = "|cFF4AFF7A→ Lvl "
                            .. lvl
                            .. ":|r |cFFFFD100"
                            .. t
                            .. "|r  ← take now"
        ...
        local mileTxt
        if playerLevel < 10 then
            mileTxt = "Milestone \226\134\146 Level 10: talent tree opens"
        elseif playerLevel < 30 then
            mileTxt = "Milestone \226\134\146 Level 30: Heroic dungeons & queued content"
        elseif playerLevel < 50 then
            mileTxt = "Milestone \226\134\146 Level 50: Delves unlock  ·  Normal dungeons scale"
        elseif playerLevel < 70 then
            mileTxt = "Milestone \226\134\146 Level 70: World quests, M0, previous-expansion raids"
        elseif playerLevel < 71 then
            mileTxt = "Milestone \226\134\146 Level 71: Hero talents unlock"
        elseif playerLevel < 81 then
            mileTxt =
                "Milestone \226\134\146 Level 81: next talent tier (verify in-game — unconfirmed for this build)"
        else
            mileTxt = "Milestone \226\134\146 Approaching max — endgame content unlocking soon"
        end
    ...
    if specID == activeSpecID and C_ClassTalents and C_ClassTalents.GetActiveConfigID then
        ...
        if cfgID and C_Traits and C_Traits.GenerateImportString then
            ...
            if exportStr and exportStr ~= "" then
                ...
                saveBtn:SetScript("OnClick", function()
                    ...
                    TA:Raw(
                        TA.LOG.OUTPUT,
                        "|cFFFFD100[TA Talents]|r Saved your current loadout as the |cFF4AFF7A"
                            .. capViewType:upper()
                            .. "|r build for "
                            .. GetSpecNameByID(capSpecID)
                            .. "."
                    )
    ...
    if specID == activeSpecID then
        ...
        local function WeaponCompatible(candidateSpecID)
            ...
            elseif need == "dual_wield" then
                ...
                local ohOK = ohEquipLoc
                    and (MELEE_1H_LOCS[ohEquipLoc] or ohEquipLoc == "INVTYPE_WEAPONOFFHAND" or false)
        ...
        if bestSpec and bestSpec ~= activeSpecID and activeScore > 0 then
            ...
            if pctDiff >= 5 then
                ...
                AddText(
                    "|cFFFF9A1AGEAR SUGGESTS:|r Your equipped stats score |cFF1EFF00"
                        .. pctDiff
                        .. "%|r higher for |cFFFFD100"
                        .. recName
                        .. "|r ("
                        .. pvxMode:upper()
                        .. " mode).",
                ...
                AddText(
                    "|cFF888780This means your current gear has more of the stats that "
                        .. recName
                        .. " values. Consider respeccing or regearing if you want to stay "
                        .. GetSpecNameByID(activeSpecID)
                        .. ".|r",
                    9,
                    0.5,
                    0.5,
                ...
                if weaponNotes[bestSpec] then
                    AddText(
                        "|cFFFF8800⚠ Weapon excluded:|r Your equipped weapon type isn't usable by "
                            .. recName
                            .. ". Score is based on armor + accessories only — you'd need a different weapon to actually respec.",
                        9,
                        0.7,
                        0.5,
                        0.2
                ...
                for _, entry in ipairs(sortedSpecs) do
                    ...
                    AddText(
                        color .. sName .. "|r: " .. U.FormatNumber(entry.score) .. " gear score" .. tag .. wpnTag,
                        9,
                        0.6,
                        0.6,
                        0.6
...
function Talents:RenderSidebar(parent, activeSpecID)
    ...
    if rinfo.role == "DAMAGER" then
        ...
        local rc = rinfo.role == "TANK" and { r = 0.29, g = 0.65, b = 1.00 } or { r = 0.29, g = 1.00, b = 0.48 }
...
function Talents:OpenImportFrame()
    ...
    importBtn:SetScript("OnClick", function()
        ...
        TA:Raw(
            TA.LOG.OUTPUT,
            string.format(
                "|cFFFFD100[TA Talents]|r Imported and saved as |cFF4AFF7A%s|r build for %s.",
                viewType:upper(),
                GetSpecNameByID(specID)
            )
        )
        cf:Hide()
        if TA.UI and TA.UI.activeTab == "talents" then
    ...
    activeBtn:SetScript("OnClick", function()
        ...
        TA:Raw(
            TA.LOG.OUTPUT,
            string.format(
                "|cFFFFD100[TA Talents]|r Captured active loadout as |cFF4AFF7A%s|r build for %s.",
                viewType:upper(),
                GetSpecNameByID(specID)
            )
        )
    ...
    statusF:SetText(
        "|cFF888780Will save as: "
            .. (self.viewBuildType or "solo"):upper()
            .. " build for "
            .. GetSpecNameByID(self.selectedSpecID or U.GetPlayerSpec() or 0)
            .. "|r"
    )
