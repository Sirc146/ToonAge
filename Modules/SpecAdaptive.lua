-- ToonAge/Modules/SpecAdaptive.lua
...
function SA:GetRoleInfo()
    ...
    if not specIndex then return "DAMAGER", "Unknown", "melee" end
    ...
    if specInfo then style = specInfo.style end
...
function SA:GetDungeonSuggestion()
    ...
    if role == "TANK" then
        return string.format("|cFF4AFF7A⚔ Tank Tip:|r Queue %s — instant queue + %.0fk XP/run (%dm avg)",
            match[3], match[5] / 1000, match[4])
    elseif role == "HEALER" then
        return string.format("|cFF4AFF7A♥ Healer Tip:|r Queue %s — fast queue + %.0fk XP/run",
            match[3], match[5] / 1000)
    else
        -- DPS: only suggest if queue time is short (low level) or XP is significantly better
        if level < 30 then
            return string.format("|cFF888780DPS Tip: %s gives ~%.0fk XP (%dm queue expected)|r",
                match[3], match[5] / 1000, math.random(5, 12))
        end
