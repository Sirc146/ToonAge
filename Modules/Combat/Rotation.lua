-- ToonAge/Modules/Combat/Rotation.lua
...
-- Picks the view (solo/aoe/st) that matches where the player actually is,
-- not just who they're grouped with. GetInstanceInfo()'s instanceType is
-- what makes this more accurate than checking group membership alone: it
-- distinguishes "partied up doing open-world content" (still aoe — you're
-- grouped, treat it as M+-style multi-target healing) from "actually inside
-- a party instance" and, more importantly, catches a raid GROUP standing in
-- open-world content vs a raid INSTANCE. instanceType == "arena"/"pvp" has
-- no dedicated rotation data in Data/Rotations.lua, so it falls through to
-- the same group-based guess as the "none"/"scenario" case below rather than
-- referencing a view that doesn't exist.
-- pcall-guarded: GetInstanceInfo can be momentarily unavailable during a
-- loading screen, and a module erroring out of Init/OnEvent gets disabled
-- after enough repeats (see TA:UpdateModules).
function Rotation:DetectView()
    ...
    if ok then
        ...
        -- "arena", "pvp", "scenario", "none" all fall through to the
        -- group-based guess below.
    end
...
function Rotation:Init()
    ...
    self.currentView = self:DetectView()
...
function Rotation:OnEvent(event, ...)
    ...
    -- GROUP_ROSTER_UPDATE catches joining/leaving a group; ZONE_CHANGED_NEW_AREA
    -- catches walking into (or out of) an instance while group composition
    -- doesn't change — e.g. entering a dungeon with a party already formed.
    -- Both are already registered in Core/Init.lua's PERSISTENT_EVENTS and
    -- dispatched here via TA:UpdateModules, so nothing else needs wiring.
    if event == "GROUP_ROSTER_UPDATE" or event == "ZONE_CHANGED_NEW_AREA" then
        local newView = self:DetectView()
        if newView ~= self.currentView then
            self.currentView = newView
            -- Re-render immediately if the tab is open, same as the
            -- talent/level branch above — previously this branch updated
            -- currentView but never refreshed the visible tab, so the old
            -- view stayed on screen until something else forced a rebuild.
            if TA.UI and TA.UI.activeTab == "rotation" then
                self:Render(TA.UI.contentChild, TA.UI.sideChild)
            end
        end
...
--- The view the next-3 bar should use right now. The content view comes from
--- where you are (dungeon = aoe, raid = st), but a dungeon boss is single
--- target and an open-world pull can be a big pack. In combat, the live enemy
--- count overrides it: 1 engaged enemy uses the single-target list, 3+ use AoE.
function Rotation:LiveView(CS)
    local view = self.currentView
    local s = CS and CS.state
    if not (s and s.inCombat) then return view end
    local n = s.aoeCount or 1
    if n >= 3 and view ~= "aoe" then return "aoe" end
    if n <= 1 and view == "aoe" then return "st" end
    return view
end
...
function Rotation:GetPredictionPriorities(specID, view)
    ...
    if soloData and soloData.priorities then
        return soloData.priorities, true
    end
...
function Rotation:RenderSidebar(parent, specID, level, groupType)
    ...
    predictBtn:SetBackdropColor(
        isBarVisible and 0.08 or 0.04,
        isBarVisible and 0.12 or 0.04,
        isBarVisible and 0.06 or 0.04,
        1
    )
    predictBtn:SetBackdropBorderColor(
        isBarVisible and 0.20 or 0.35,
        isBarVisible and 0.92 or 0.28,
        isBarVisible and 0.40 or 0.06,
        0.8
    )
    ...
    predictLbl:SetTextColor(
        ...
        isBarVisible and 0.90 or 0.44,
        isBarVisible and 0.87 or 0.30,
        1
    )
...
function Rotation:InitPredictBar()
    ...
    if savedBar and savedBar.scale then
        bar:SetScale(savedBar.scale)
    end
    ...
    triage:SetPoint("BOTTOM", bar, "TOP", 0, 2)
    triage:Hide()
    self.triageLabel = triage
    ...
    bar:SetScript("OnUpdate", function(_, elapsed)
        ...
        if self._predictThrottle < PREDICT_UPDATE_HZ then
            return
        end
...
function Rotation:UpdatePrediction()
    ...
    if self.triageLabel then
        local g = CS.state and CS.state.group
        local isHealer = (U.GetPlayerRole and U.GetPlayerRole() == "HEALER")
        local t = isHealer and g and g.triage
        if t and IsInGroup and IsInGroup() then
            local pct = math.floor(t.pct + 0.5)
            local col = (pct < 40 and "|cFFFF4444") or (pct < 70 and "|cFFFF9A1A") or "|cFF4AFF7A"
            self.triageLabel:SetText(string.format("Heal: %s%s|r %s%d%%|r%s", col, t.name, col, pct,
                t.role == "TANK" and " |cFF888780(tank)|r" or ""))
            self.triageLabel:Show()
        else
            self.triageLabel:Hide()
        end
    end
...
function Rotation:TogglePredictBar()
    ...
    if self.predictBar:IsVisible() then
        ...
        if TA.charDB then
            TA.charDB.predictBar = TA.charDB.predictBar or {}
            TA.charDB.predictBar.visible = false
        end
...
function Rotation:RenderPredictionInline(content, y, w, padL, rotData, level)
    ...
    if not priorities then
        priorities = self:GetPredictionPriorities(self.currentSpecID or U.GetPlayerSpec(), self.currentView)
    end
    if not priorities then
        return y
    end
