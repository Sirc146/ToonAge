-- ToonAge/Modules/Infrastructure/Onboarding.lua
...
function Onboarding:RunSilentInherit()
    ...
    C_Timer.After(3, function()
        ...
        if Rot and Rot.predictBar and TA.charDB.predictBar and TA.charDB.predictBar.visible then
            Rot.predictBar:Show()
        end
        ...
        TA:Raw(
            TA.LOG.OUTPUT,
            "|cFFFFD100[ToonAge]|r New character detected. Inheriting |cFFFFD100"
                .. label
                .. "|r setup.  |cFF888780/ta onboard for the full setup"
...
function Onboarding:CompleteFlow()
    ...
    C_Timer.After(0.4, function()
        ...
        local optedOut = TA.charDB and TA.charDB.altQuickStart and TA.charDB.altQuickStart.disabled
...
Onboarding.SlashCommands = {
    onboard = function(self, args)
        ...
        if BEHAVIOR_DESC[sub] then
            if TA.db then
                TA.db.newCharBehavior = sub
            end
            TA:Raw(
                TA.LOG.OUTPUT,
                "|cFFFFD100[ToonAge]|r New characters: |cFFFFD100" .. sub .. "|r — " .. BEHAVIOR_DESC[sub] .. "."
            )
            return
        end
        ...
        if sub == "account" or sub == "character" then
            ...
            if TA.db then
                TA.db.newCharBehavior = mapped
            end
            TA:Raw(
                TA.LOG.OUTPUT,
                "|cFFFFD100[ToonAge]|r |cFF888780Scopes were replaced by modes.|r "
                    .. "New characters: |cFFFFD100"
                    .. mapped
                    .. "|r — "
        ...
        if sub == "preset" then
            if rest ~= "auto" and rest ~= "manual" then
                ...
                TA:Raw(
                    TA.LOG.OUTPUT,
                    "|cFFFFD100[ToonAge]|r Inherited preset: |cFFFFD100"
                        .. cur
            ...
            if TA.db then
                TA.db.defaultPreset = rest
            end
            TA:Raw(
                TA.LOG.OUTPUT,
                "|cFFFFD100[ToonAge]|r New characters will inherit the |cFFFFD100" .. rest .. "|r preset."
            )
            return
        end
        ...
        if sub == "reset" then
            if TA.charDB then
                TA.charDB.onboarded = nil
            end
            TA:Raw(
                TA.LOG.OUTPUT,
                "|cFFFFD100[ToonAge]|r This character will be treated as new next login. "
        ...
        if sub ~= "" then
            TA:Raw(
                TA.LOG.OUTPUT,
                "|cFFFFD100[ToonAge]|r Unknown: /ta onboard "
                    .. sub
        ...
        TA:Raw(
            TA.LOG.OUTPUT,
            "|cFFFFD100[ToonAge]|r Setup wizard — new characters are set to |cFFFFD100"
                .. mode
                .. "|r.  |cFF888780/ta onboard wizard|inherit|off to change.|r"
        )
        self:ShowPopup()
    end,
}
...
function Onboarding:ApplyPreset(preset)
    ...
    if TA.db then
        TA.db.defaultPreset = (preset == "auto") and "auto" or "manual"
    end

    if preset == "auto" then
        TA:Raw(
            TA.LOG.OUTPUT,
            "|cFF4AFF7A[ToonAge]|r Full Auto enabled! Quests auto-accept, cutscenes skip, combat bar active."
        )
