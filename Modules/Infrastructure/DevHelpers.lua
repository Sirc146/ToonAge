...
-- ── /ta spellaudit — verify every data-file spellID against the live client ──
-- Data files are hand-authored, and a wrong spellID fails silently: IsSpellKnown
-- is simply false, so the entry never appears in the prediction bar and nobody
-- notices. (Found this way in 12.1: Kill Shot pointed at Serpent Sting's ID,
-- Eternity Surge at Firestorm's, Tentacle Slam at an NPC ability.) This asks the
-- running client for each ID's real name and lists every ID that doesn't exist
-- or whose name doesn't match what the data file claims.
--
-- Usage: /ta spellaudit        (run out of combat; results open in a copy window)
...
local function SpellAudit()
    ...
    local checked, missing, mismatch = 0, {}, {}
    local seen = {}

    local function check(spellID, claimed, where)
        if type(spellID) ~= "number" or spellID <= 0 then return end
        local key = spellID .. "|" .. tostring(claimed)
        if seen[key] then return end
        seen[key] = true
        checked = checked + 1
        local ok, real = pcall(getName, spellID)
        if not ok or not real or real == "" then
            missing[#missing + 1] = string.format("%-8d %-28s %s", spellID, tostring(claimed), where)
        elseif claimed and NormName(real) ~= NormName(claimed)
            and not NormName(claimed):find(NormName(real), 1, true)
            and not NormName(real):find(NormName(claimed), 1, true) then
            mismatch[#mismatch + 1] = string.format("%-8d data=%-26s client=%-26s %s",
                spellID, tostring(claimed), tostring(real), where)
        end
    end

    local function walkList(list, where)
        if type(list) ~= "table" then return end
        for _, e in ipairs(list) do
            if type(e) == "table" then check(e.spellID, e.name, where) end
        end
    end

    local D = TA.Data or {}
    if type(D.Rotations) == "table" then
        for specID, spec in pairs(D.Rotations) do
            if type(specID) == "number" and type(spec) == "table" then
                for view, v in pairs(spec) do
                    if type(v) == "table" then
                        local w = "Rotations[" .. specID .. "]." .. tostring(view)
                        walkList(v.priorities, w)
                        walkList(v.cooldowns, w .. ".cd")
                        walkList(v.chain, w .. ".chain")
                    end
                end
            end
        end
    end
    if type(D.TalentsPvP) == "table" then
        for specID, spec in pairs(D.TalentsPvP) do
            if type(specID) == "number" and type(spec) == "table" and type(spec.nodes) == "table" then
                for id, node in pairs(spec.nodes) do
                    check(id, type(node) == "table" and node.pick or nil, "TalentsPvP[" .. specID .. "]")
                end
            end
        end
    end
    if type(D.Spells) == "table" then
        for group, ids in pairs(D.Spells) do
            if type(ids) == "table" then
                for const, id in pairs(ids) do
                    check(id, nil, "Spells." .. tostring(group) .. "." .. tostring(const))
                end
            end
        end
    end

    -- ── Aura IDs inside rotation conditions (`when = C.HasBuff(...)` etc.) ──
    local auraRefs, auraMissing = {}, {}
    local C = D.RotationConditions
    if C and C.AuraRefs and type(D.Rotations) == "table" then
        local seenRef = {}
        for specID, spec in pairs(D.Rotations) do
            if type(specID) == "number" and type(spec) == "table" then
                for view, v in pairs(spec) do
                    if type(v) == "table" then
                        for _, listName in ipairs({ "priorities", "cooldowns" }) do
                            for _, e in ipairs(v[listName] or {}) do
                                if type(e) == "table" and e.when then
                                    for _, ref in ipairs(C.AuraRefs(e.when)) do
                                        local key = specID .. "|" .. tostring(e.name) .. "|" .. ref.kind .. "|" .. ref.id
                                        if not seenRef[key] then
                                            seenRef[key] = true
                                            checked = checked + 1
                                            local ok, real = pcall(getName, ref.id)
                                            local line = string.format("%-6s %-26s %-13s %-8d %s",
                                                tostring(specID), tostring(e.name), ref.kind, ref.id,
                                                (ok and real) or "?? MISSING")
                                            if not ok or not real or real == "" then
                                                auraMissing[#auraMissing + 1] = line .. "   Rotations[" .. specID .. "]." .. tostring(view)
                                            else
                                                auraRefs[#auraRefs + 1] = line
                                            end
                                        end
                                    end
                                end
    ...
    table.sort(auraMissing); table.sort(auraRefs)

    -- ── IDs hardcoded in modules ──────────────────────────────────────────
    local CS = TA:GetModule("CombatState")
    if CS and CS.GCD_SPELL_ID then check(CS.GCD_SPELL_ID, "Global Cooldown", "CombatState.GCD_SPELL_ID") end
    local TR = TA:GetModule("TravelRouter")
    if TR and type(TR.CLASS_TELEPORTS) == "table" then
        for _, t in ipairs(TR.CLASS_TELEPORTS) do
            check(t.spellID, t.label, "TravelRouter.CLASS_TELEPORTS")
        end
    end
    local currencyLines = {}
    local G = TA:GetModule("Gear")
    if G and type(G.UPGRADE_CURRENCIES) == "table" and C_CurrencyInfo and C_CurrencyInfo.GetCurrencyInfo then
        for _, c in ipairs(G.UPGRADE_CURRENCIES) do
            checked = checked + 1
            local ok, info = pcall(C_CurrencyInfo.GetCurrencyInfo, c.id)
            local real = ok and info and info.name
            if not real or real == "" then
                currencyLines[#currencyLines + 1] = string.format("%-8d %-26s MISSING on this client   Gear.UPGRADE_CURRENCIES", c.id, c.name)
            elseif NormName(real) ~= NormName(c.name) then
                currencyLines[#currencyLines + 1] = string.format("%-8d data=%-21s client=%s   Gear.UPGRADE_CURRENCIES", c.id, c.name, real)
            end
        end
    end

    -- ── Current spec: hero trees and rotation spells this spec can actually have ──
    local specLines = {}
    local specIndex = GetSpecialization and GetSpecialization()
    local specID, specName
    if specIndex then specID, specName = GetSpecializationInfo(specIndex) end
    local configID = C_ClassTalents and C_ClassTalents.GetActiveConfigID and C_ClassTalents.GetActiveConfigID()
    if specID and configID and C_Traits then
        -- Every spell the class/spec/hero tree can grant
        local treeSpells, heroNames = {}, {}
        local okCfg, cfg = pcall(C_Traits.GetConfigInfo, configID)
        local treeID = okCfg and cfg and cfg.treeIDs and cfg.treeIDs[1]
        if treeID then
            local okNodes, nodes = pcall(C_Traits.GetTreeNodes, treeID)
            for _, nodeID in ipairs(okNodes and nodes or {}) do
                local okN, node = pcall(C_Traits.GetNodeInfo, configID, nodeID)
                if okN and node and node.isVisible ~= false then
                    for _, entryID in ipairs(node.entryIDs or {}) do
                        ...
                        if okE and entry and entry.definitionID then
                            okD, def = pcall(C_Traits.GetDefinitionInfo, entry.definitionID)
                        end
                        if okD and def then
                            if def.spellID then treeSpells[def.spellID] = true end
                            if def.overriddenSpellID then treeSpells[def.overriddenSpellID] = true end
                        end
                    end
                end
            end
        ...
        if C_ClassTalents.GetHeroTalentSpecsForClassSpec then
            local okH, subTrees = pcall(C_ClassTalents.GetHeroTalentSpecsForClassSpec, configID, specID)
            for _, subTreeID in ipairs(okH and subTrees or {}) do
                local okS, info = pcall(C_Traits.GetSubTreeInfo, configID, subTreeID)
                if okS and info and info.name then heroNames[NormName(info.name)] = info.name end
            end
        end

        local function Available(id)
            if treeSpells[id] then return true end
            if IsPlayerSpell and IsPlayerSpell(id) then return true end
            if C_SpellBook and C_SpellBook.IsSpellInSpellBook then
                local okB, inBook = pcall(C_SpellBook.IsSpellInSpellBook, id)
                if okB and inBook then return true end
            end
            return false
        end

        local rot = D.Rotations and D.Rotations[specID]
        local notAvail, seenA = {}, {}
        for view, v in pairs(type(rot) == "table" and rot or {}) do
            if type(v) == "table" then
                for _, listName in ipairs({ "priorities", "cooldowns", "chain" }) do
                    for _, e in ipairs(v[listName] or {}) do
                        if type(e) == "table" and type(e.spellID) == "number" and not seenA[e.spellID] then
                            seenA[e.spellID] = true
                            if not Available(e.spellID) then
                                notAvail[#notAvail + 1] = string.format("%-8d %-28s Rotations[%d].%s", e.spellID, tostring(e.name), specID, tostring(view))
                            end
                        end
                    end
                end
        ...
        local T = D.Talents
        local build = T and T.GetBySpecID and T:GetBySpecID(specID)
        local heroBad = {}
        if build and build.builds and next(heroNames) then
            for kind, b in pairs(build.builds) do
                local n = b.name and NormName((b.name:gsub("%s*%(PvP%)", "")))
                if n and n ~= "" and not heroNames[n] then
                    heroBad[#heroBad + 1] = string.format("Talents[%d].%s = %q is not a hero tree for this spec", specID, kind, b.name)
                end
            end
        end
        local heroList = {}
        for _, name in pairs(heroNames) do heroList[#heroList + 1] = name end
        table.sort(heroList)

        specLines[#specLines + 1] = string.format("Spec %s (%d) · hero trees on client: %s", tostring(specName), specID,
            #heroList > 0 and table.concat(heroList, ", ") or "none read")
        for _, l in ipairs(heroBad) do specLines[#specLines + 1] = l end
        specLines[#specLines + 1] = string.format("%d rotation spells not in your spellbook or talent tree (check: removed, renamed, or granted by a proc/override):", #notAvail)
        for _, l in ipairs(notAvail) do specLines[#specLines + 1] = "  " .. l end
        specLines[#specLines + 1] = "Run this on every class/spec you play to cover them all."
    else
        specLines[#specLines + 1] = "No active spec/talent config — log in on a character with a spec to run this part."
    end
    ...
    local out = {
        ...
        "",
        "== IDs that do not exist on this client ==",
    }
    for _, l in ipairs(missing) do out[#out + 1] = l end
    out[#out + 1] = ""
    out[#out + 1] = "== IDs whose client name differs from the data file =="
    for _, l in ipairs(mismatch) do out[#out + 1] = l end
    out[#out + 1] = ""
    out[#out + 1] = "== Upgrade currencies (Gear tab) =="
    if #currencyLines == 0 then out[#out + 1] = "all match" end
    for _, l in ipairs(currencyLines) do out[#out + 1] = l end
    out[#out + 1] = ""
    out[#out + 1] = "== Current spec check =="
    for _, l in ipairs(specLines) do out[#out + 1] = l end
    out[#out + 1] = ""
    out[#out + 1] = "== Condition aura IDs that do not exist =="
    for _, l in ipairs(auraMissing) do out[#out + 1] = l end
    out[#out + 1] = ""
    out[#out + 1] = string.format("== All %d condition aura references (spec, ability, check, aura id, client name) ==", #auraRefs)
    for _, l in ipairs(auraRefs) do out[#out + 1] = l end
    p(string.format("Spell audit: %d checked, %d missing, %d mismatched, %d condition auras missing, %d currency issues.",
        checked, #missing, #mismatch, #auraMissing, #currencyLines))
    DH:ShowExport("Spell Audit", out)
end

DH.SlashCommands["spellaudit"] = function(self) SpellAudit() end
