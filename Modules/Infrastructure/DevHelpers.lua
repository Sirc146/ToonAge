-- ToonAge/Modules/Infrastructure/DevHelpers.lua
...
local function ExportFrame()
    ...
    local scroll = CreateFrame("ScrollFrame", "TADevExportScroll", f, "UIPanelScrollFrameTemplate")
...
function DH:ShowExport(label, body)
    ...
    f.heading:SetText(string.format("|cFFFFD100%s|r  |cFF888888(%d lines)|r", label or "ToonAge Export", newlines + 1))
...
function DH:DumpRecording()
    ...
    for i, step in ipairs(r.steps) do
        ...
        lines[#lines + 1] =
            string.format("            coord   = { map = %d, x = %.2f, y = %.2f },", step.map, step.x, step.y)
        lines[#lines + 1] = "        },"
    end
...
local function PrintGuideList()
    ...
    for i, g in ipairs(list) do
        p(string.format("  |cFFFFD100%d.|r  %-36s  lvl %d-%d  map=%d", i, g.title, g.minLevel, g.maxLevel, g.zone))
    end
...
SlashCmdList["TAQUESTSCAN"] = function()
    ...
    if count == 0 then
        p("No active quests (only headers).")
        return
    end
    ...
    local out = {
        ...
        string.format("-- %d active quest(s), captured at player position %.2f, %.2f", count, px, py),
        "--",
        ...
        for _, cdb in pairs(TA.db.char) do
            if type(cdb) == "table" and type(cdb.coordLog) == "table" then
                for _, e in ipairs(cdb.coordLog) do
                    ...
                    if not dup then
                        TA.db.coordLog[#TA.db.coordLog + 1] = e
                    end
                end
                cdb.coordLog = nil
            end
...
local function CoordLogDump()
    ...
    for i, e in ipairs(log) do
        lines[#lines + 1] = string.format("%2d. %-28s map=%-6d %s", i, e.zone or "?", e.map or 0, e.parents or "")
    end
...
SlashCmdList["TACOORD"] = function(msg)
    ...
    local mapID = C_Map.GetBestMapForUnit("player")
    if not mapID then
        p("No map data available here.")
        return
    end
    local pos = C_Map.GetPlayerMapPosition(mapID, "player")
    if not pos then
        p("Cannot read position on this map.")
        return
    end
    local x, y = pos:GetXY()
    local info = C_Map.GetMapInfo(mapID) or {}
    local zoneName = info.name or "Unknown"
    p(string.format("[%s  map=%d]  coord = { map = %d, x = %.2f, y = %.2f },", zoneName, mapID, mapID, x, y))
    ...
    local parentStr = #chain > 0 and table.concat(chain, "  >  ") or "none (top-level map)"
    p("  parents: " .. parentStr)
...
SlashCmdList["TATELEPORTS"] = function()
    ...
    local function report(name, spellID)
        if not name or not spellID or seen[spellID] then
            return
        end
        if
            not (
                name:find("Teleport")
                or name:find("Portal")
                or name:find("Dreamwalk")
                or name:find("Recall")
                or name:find("Death Gate")
                or name:find("Zen Pilgrimage")
            )
        then
            return
        end
    ...
    if C_SpellBook and C_SpellBook.GetNumSpellBookSkillLines then
        ...
        for i = 1, numLines do
            ...
            if line then
                ...
                for j = from, to do
                    local ok, info = pcall(C_SpellBook.GetSpellBookItemInfo, j, Enum.SpellBookSpellBank.Player)
                    if ok and info then
                        report(info.name, info.spellID)
                    end
                end
            end
        end
...
SlashCmdList["TAWEEKLY"] = function()
    ...
    if Enum and Enum.WeeklyRewardChestThresholdType then
        ...
        for i = 1, 3 do
            table.insert(typeList, { label = "raw " .. i, value = i })
        end
    end
...
SlashCmdList["TADEV"] = function()
    ...
    if not specIndex then
        p("No active specialization detected.")
        return
    end
...
DH.SlashCommands["talentscan"] = function(self)
    TalentScan()
end
...
function DH:Init()
    ...
    if TA.charDB.recorder and TA.charDB.recorder.steps and #TA.charDB.recorder.steps > 0 then
        p(
            string.format(
                "Quest recorder has %d step(s) from a previous session.  /tarecord dump to export, /tarecord clear to reset.",
                #TA.charDB.recorder.steps
            )
        )
    end
end
...
DH.SlashCommands["secretprobe"] = function()
    ...
    if not ok or not aura then
        ...
        try("tonumber(v)", function()
            return tonumber(raw)
        end)
        try("tonumber(tostring(v))", function()
            return tonumber(tostring(raw))
        end)
        try("v + 0", function()
            return raw + 0
        end)
        try("math.floor(v)", function()
            return math.floor(raw)
        end)
        try("format('%d', v)", function()
        ...
        try("#tostring(v)", function()
            return #tostring(raw)
        end)
        try("tostring(v):match(%d+)", function()
            return tostring(raw):match("%d+")
        end)
        try("tonumber(match)", function()
        ...
        out(("|cFFFFD100  -- all buffs (inCombat=%s) --|r"):format(tostring(InCombatLockdown() and true or false)))
        local total, failed = 0, 0
        for i = 1, 40 do
            local aok, a = pcall(C_UnitAuras.GetBuffDataByIndex, "player", i)
            if not aok or not a then
                break
            end
            total = total + 1
            ...
            out2(
                ("    %2d  tostring=|cFFFFD100%-9s|r SafeNum=%s%s|r  %s"):format(
                    i,
                    tostring(a.spellId),
                    id == 0 and "|cFFFF4444" or "|cFF4AFF7A",
                    tostring(id),
                    tostring(a.name or "?")
                ),
                ("    %2d  SafeNum=%-9s secret=%-5s name=%s"):format(i, tostring(id), tostring(id == 0), safeName)
            )
        end
        ...
        for _, entry in ipairs(KNOWN) do
            ...
            local aOK, aRes = pcall(function()
                for i = 1, 40 do
                    ...
                    if not sok or not a then
                        break
                    end
                    if a.spellId == wantID then
                        return "MATCHED at slot " .. i
                    end
                end
            ...
            out(("      A ==literal  %s%s|r"):format(aOK and "|cFF4AFF7A" or "|cFFFF4444", tostring(aRes)))
            out(("      B byID       %s%s|r"):format(bOK and "|cFF4AFF7A" or "|cFFFF4444", tostring(bRes)))
        end
    ...
    if CS and CS.state then
        local nb, nd = 0, 0
        for _ in pairs(CS.state.buffs or {}) do
            nb = nb + 1
        end
        for _ in pairs(CS.state.debuffs or {}) do
            nd = nd + 1
        end
    ...
    if cdOk and cd then
        out(
            ("  cd.duration       : raw=|cFFFFD100%s|r  SafeNum=|cFFFFD100%s|r"):format(
                tostring(cd.duration),
                tostring(U.SafeNum(cd.duration))
            )
        )
    end
    out("|cFFFFD100━━━━━━━━━━━━━━━━━━━━━━━━━|r")
    ...
    if TA.db then
        ...
        while #TA.db.probeLog > 3 do
            table.remove(TA.db.probeLog)
        end
        TA:Raw(
            TA.LOG.OUTPUT,
            (
...
DH.SlashCommands["auradump"] = function()
    ...
    if TA.db then
        ...
        while #TA.db.auraLog > 8 do
            table.remove(TA.db.auraLog)
        end
        TA:Raw(
            TA.LOG.OUTPUT,
            ("|cFF4AFF7ASaved (%d dump(s) kept).|r |cFF888780/reload to flush to disk.|r"):format(#TA.db.auraLog)
        )
    end
end
