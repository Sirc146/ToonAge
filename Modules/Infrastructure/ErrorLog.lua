...
-- Fix (2026-09-13): this handler runs on EVERY Lua error in the client, not
-- just ToonAge's own -- seterrorhandler() below replaces the game's handler
-- entirely, so an error from any other addon still reaches IsOurError/the
-- stack-trace check first. debugstack() is not free, and if some OTHER
-- addon starts throwing on every event in a tight loop (a bug that has
-- nothing to do with ToonAge), this handler doing that work just as many
-- times in one frame is exactly what can itself trip Blizzard's "insecure
-- scripts exceeded execution limit" watchdog -- which then blames ToonAge,
-- since ToonAge's code was executing when the clock ran out. Reported as a
-- 73-count ToonAge script-timeout that coincided with a different addon
-- (TomTomTargetArrow) erroring on every PLAYER_TARGET_CHANGED. The circuit
-- breaker below caps how much extra work this handler will do during any
-- short burst -- the original handler is still always chained below, so
-- other addons' own error reporting (BugSack etc.) keeps working normally;
-- only ToonAge's own debugstack/logging work gets throttled once a burst is
-- clearly underway, so a runaway error anywhere else in the client can no
-- longer cascade into a ToonAge script timeout.
local ERROR_BURST_WINDOW = 1.0  -- seconds
local ERROR_BURST_LIMIT  = 15   -- max handled per window before throttling
local burstWindowStart = 0
local burstCount = 0
local lastMsg, lastMsgCount = nil, 0

local function ToonAgeErrorHandler(msg)
    local now = GetTime()
    if now - burstWindowStart > ERROR_BURST_WINDOW then
    ...
    if msg == lastMsg then
        ...
        lastMsg, lastMsgCount = msg, 1
    end

    -- Do our own (non-trivial) work only while this isn't a burst/repeat --
    -- once it clearly is, skip straight to chaining the original handler.
    if burstCount <= ERROR_BURST_LIMIT and lastMsgCount <= 3 then
