-- ToonAge/Modules/Progression/AltQuickStart.lua
...
function AQS:PopulatePanel()
    ...
    dontShow:SetChecked((TA.charDB and TA.charDB.altQuickStart and TA.charDB.altQuickStart.disabled) and true or false)
    ...
    dontShow:SetScript("OnEnter", function(selfBtn)
        ...
        GameTooltip:AddLine("Stops this panel opening by itself for this character.", 0.8, 0.8, 0.8, true)
        GameTooltip:AddLine("You can still open it any time with /ta quickstart.", 0.6, 0.6, 0.6, true)
        GameTooltip:Show()
    end)
    dontShow:SetScript("OnLeave", function()
        GameTooltip:Hide()
    end)
    ...
    dontShow:SetScript("OnClick", function(selfBtn)
        ...
        if off then
            ...
            if QT and QT.ShowToast then
                QT:ShowToast("Quick Start disabled for this character")
            end
        end
    end)
    ...
    if
        ...
        and TA.charDB.customBuilds[spec.specID].solo
    then
    ...
    if buildString and buildString ~= "" then
        ...
        importBtn:SetScript("OnClick", function()
            if InCombatLockdown() then
                local QT = TA:GetModule("QuestTracker")
                if QT and QT.ShowToast then
                    QT:ShowToast("Can't change talents in combat")
                end
                return
            end
        ...
        importBtn:SetScript("OnEnter", function(btn)
            btn:SetBackdropColor(0.10, 0.16, 0.10, 1)
        end)
        importBtn:SetScript("OnLeave", function(btn)
            btn:SetBackdropColor(0.06, 0.10, 0.06, 1)
        end)
    ...
    for _, entry in ipairs(priorities) do
        if shown >= MAX_ROTATION_ICONS then
            break
        end
    ...
    local nameCount = 0
    for _, entry in ipairs(priorities) do
        if nameCount >= MAX_ROTATION_ICONS then
            break
        end
    ...
    C_Timer.After(2, function()
        ...
        if IsReturningAlt() then
            ...
            if QT and QT.ShowToast then
                ...
                TA:Raw(
                    TA.LOG.OUTPUT,
                    "|cFFFFD100[ToonAge]|r Welcome back — |cFFFFD100/ta quickstart|r for your catch-up."
                )
            end
        end
