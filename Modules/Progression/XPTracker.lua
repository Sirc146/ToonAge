-- ToonAge/Modules/Progression/XPTracker.lua
...
function XP:GetRate()
    ...
    if timeDiff < 5 then
        return 0, 0
    end -- need at least 5 seconds of data
...
function XP:GetETAString()
    ...
    if eta < 60 then
        return string.format("|cFF4AFF7A<1m to %d|r", GetLevel() + 1)
    end
    if eta < 3600 then
        return string.format("|cFF4AFF7ALvl %d in %dm|r", GetLevel() + 1, math.floor(eta / 60))
    end
    return string.format(
        "|cFFFFD100Lvl %d in %dh %dm|r",
        GetLevel() + 1,
        math.floor(eta / 3600),
        math.floor((eta % 3600) / 60)
    )
end
...
function XP:GetAltComparison()
    if not TA.db or not TA.db.xpHistory then
        return nil
    end
...
function XP:Init()
    if IsMaxLevel() then
        return
    end -- no point tracking at max level
...
XP.SlashCommands = {
    xp = function(self)
        ...
        TA:Raw(
            ...
            string.format(
                "  Session: %s XP in %dm",
                U.FormatNumber and U.FormatNumber(s.sessionXP) or tostring(s.sessionXP),
                mins
            )
        )
        ...
        if alts and #alts > 0 then
            ...
            for _, alt in ipairs(alts) do
                ...
                TA:Raw(
                    TA.LOG.OUTPUT,
                    string.format("    %s — %s%s%.0f%%|r faster", alt.name, color, sign, alt.diffPct)
                )
            end
        end
    end,
