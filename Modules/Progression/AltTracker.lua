-- ToonAge/Modules/Progression/AltTracker.lua
...
local function GetClassHex(class)
    ...
    if cc then
        return cc.colorStr or "ffffffff"
    end
    return "ffffffff"
end
...
local function FormatTimeSince(ts)
    if not ts or ts == 0 then
        return COL_GREY .. "—" .. CLOSE
    end
    local elapsed = time() - ts
    if elapsed < 60 then
        return COL_GREEN .. "Now" .. CLOSE
    end
...
function AltTracker:SnapshotCurrentCharacter()
    if not TA.db or not TA.charKey then
        return
    end
...
function AltTracker:Render(content, sidebar)
    ...
    table.sort(chars, function(a, b)
        if a.key == TA.charKey then
            return true
        end
        if b.key == TA.charKey then
            return false
        end
    ...
    local infoRows = {
        {
            label = "Level",
            fn = function(d)
                return tostring(d.level or "?")
            end,
        },
        {
            label = "iLvl",
            fn = function(d)
                return tostring(d.ilvl or "—")
            end,
        },
        {
            label = "Last Seen",
            fn = function(d)
                return FormatTimeSince(d.lastSeen)
            end,
        },
    }
