...
-- ── Reward detection ──────────────────────────────────────────────────────────
-- Returns: { type = "gear"|"gold"|"rep"|"mats"|"other", value, text, types = { gear=true, ... } }
-- `type` is the headline reward shown on the row; `types` is every reward kind the
-- quest pays, so a quest with gold AND reputation appears under both filters.
local function ClassifyReward(questID)
    local out = { type = "other", value = 0, text = "", types = {} }
    if not questID then return out end

    -- Ask the client to load reward data; rows fill in on the next refresh.
    ...
    -- Items: gear (equippable) and profession materials (item class 7, Tradeskill)
    local gearIlvl, gearName, matsName, matsCount = 0, nil, nil, 0
    local numItems = 0
    ...
    for i = 1, numItems do
        if GetQuestLogRewardInfo then
            local ok, name, _, count, _, _, itemID = pcall(GetQuestLogRewardInfo, i, questID)
            if ok and itemID then
                local equipLoc, classID
                if C_Item and C_Item.GetItemInfoInstant then
                    local _, _, _, eLoc, _, cID = C_Item.GetItemInfoInstant(itemID)
                    equipLoc, classID = eLoc, cID
                elseif GetItemInfoInstant then
                    local _, _, _, eLoc, _, cID = GetItemInfoInstant(itemID)
                    equipLoc, classID = eLoc, cID
                end
                if equipLoc and equipLoc ~= "" and equipLoc ~= "INVTYPE_NON_EQUIP_IGNORE" then
                    local ilvl = 0
                    if C_Item and C_Item.GetDetailedItemLevelInfo then
                        local okL, lv = pcall(C_Item.GetDetailedItemLevelInfo, itemID)
                        if okL and lv then ilvl = lv end
                    end
                    if ilvl >= gearIlvl then gearIlvl, gearName = ilvl, name or "Gear" end
                elseif classID == 7 then
                    matsName, matsCount = name or "Materials", matsCount + (count or 1)
                end
            end
        end
    end

    -- Reputation (Major Factions / renown), then any other currency
    local repText
    if C_QuestLog and C_QuestLog.GetQuestLogMajorFactionReputationRewards then
        local ok, reps = pcall(C_QuestLog.GetQuestLogMajorFactionReputationRewards, questID)
        if ok and type(reps) == "table" and #reps > 0 then
            local r = reps[1]
            local factionName
            if r.factionID and C_MajorFactions and C_MajorFactions.GetMajorFactionData then
                local okF, data = pcall(C_MajorFactions.GetMajorFactionData, r.factionID)
                factionName = okF and data and data.name
            end
            repText = (r.rewardAmount and (r.rewardAmount .. " ") or "") .. (factionName or "Reputation")
        end
    end
    local currText
    if C_QuestLog and C_QuestLog.GetQuestRewardCurrencies then
        ...
        if ok and type(currencies) == "table" and #currencies > 0 then
            ...
            currText = (first.totalRewardAmount and (first.totalRewardAmount .. " ") or "") .. (first.name or "Currency")
        end
    end

    if gearName then out.types.gear = true end
    if money >= 10000 then out.types.gold = true end
    if repText then out.types.rep = true end
    if matsName then out.types.mats = true end

    -- Headline: gear > rep > gold > mats > other currency
    if gearName then
        out.type, out.value, out.text = "gear", gearIlvl, gearName .. (gearIlvl > 0 and (" (" .. gearIlvl .. ")") or "")
    elseif repText then
        out.type, out.value, out.text = "rep", 1, repText
    elseif money >= 10000 then
        local goldAmt = math.floor(money / 10000)
        out.type, out.value, out.text = "gold", goldAmt, goldAmt .. "g"
    elseif matsName then
        out.type, out.value, out.text = "mats", matsCount, matsName
    elseif currText then
        out.type, out.value, out.text = "other", 0, currText
    end
    return out
end
    ...
    for _, poi in ipairs(raw) do
        ...
        if qid and not poi.inProgress then
            ...
            local passFilter = (filter == FILTER.ALL)
                            or (reward.types and reward.types[filter])
...
function WQ:GetZoneSummary(mapID)
    ...
    mapID = mapID or (C_Map and C_Map.GetBestMapForUnit and C_Map.GetBestMapForUnit("player"))
    if not mapID then return summary end

    local raw = GetQuestsOnMap(mapID)
    if not raw then return summary end

    for _, poi in ipairs(raw) do
        if poi.questID and not poi.inProgress then
            local reward = ClassifyReward(poi.questID)
            for kind in pairs(reward.types) do
                summary[kind] = (summary[kind] or 0) + 1
            end
            if not next(reward.types) then summary.other = summary.other + 1 end
...
function WQ:RenderSidebar(sidebar, filter)
    ...
    filter = filter or (TA.charDB and TA.charDB.wqFilter) or FILTER.ALL

    local y = -10
    local w = sidebar:GetWidth() - 16
    if not w or w <= 40 then w = 186 end
    ...
    -- Filter buttons (click to filter; choice is remembered per character)
    local filters = {
        { id = FILTER.ALL,  label = "All",  count = summary.total },
        { id = FILTER.GEAR, label = "Gear", count = summary.gear },
        { id = FILTER.GOLD, label = "Gold", count = summary.gold },
        { id = FILTER.REP,  label = "Rep",  count = summary.rep },
        { id = FILTER.MATS, label = "Mats", count = summary.mats },
    }
    local gap = 3
    local bw = math.floor((w - gap * (#filters - 1)) / #filters)
    local bx = 8
    for _, filt in ipairs(filters) do
        ...
        if not active and (filt.count or 0) == 0 and filt.id ~= FILTER.ALL then
            lbl:SetTextColor(0.45, 0.45, 0.45, 1)
        end
        btn:SetScript("OnClick", function()
            if TA.charDB then TA.charDB.wqFilter = filt.id end
            if TA.UI and TA.UI.Refresh then TA.UI:Refresh() end
        end)
        btn:SetScript("OnEnter", function(self)
            GameTooltip:SetOwner(self, "ANCHOR_BOTTOM")
            GameTooltip:SetText(filt.label, 1, 0.82, 0)
            GameTooltip:AddLine(string.format("%d world quest%s in this zone", filt.count or 0,
                (filt.count or 0) == 1 and "" or "s"), 0.9, 0.9, 0.9)
            GameTooltip:Show()
        end)
    ...
    if #quests == 0 then
        ...
        empty:SetText(COL_GREY .. ((filter ~= FILTER.ALL and summary.total > 0)
            and "No world quests with that reward here.\nPick another filter or zone."
            or  "No world quests found.\nTravel to a max-level zone.") .. CLOSE)
        ...
        for i = 1, maxShow do
            ...
            local rCol = q.reward.type == "gear" and COL_BLUE
                      ...
                      or q.reward.type == "rep"  and COL_GREEN
                      or COL_GREY
