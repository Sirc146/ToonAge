-- ToonAge/Modules/Progression/DungeonTalents.lua
...
function DungeonTalents:CheckDungeonTalents()
    ...
    if self.lastInstance == instanceName then
        return
    end
    ...
    if
        TA.charDB
        and TA.charDB.dungeonTalents
        and TA.charDB.dungeonTalents.dismissed
        and TA.charDB.dungeonTalents.dismissed[instanceName]
    then
        return
    end
    ...
    if not recommendedString or recommendedString == "" then
        return
    end
    ...
    StaticPopupDialogs["TOONAGE_TALENT_SWITCH"] = {
        ...
        OnAccept = function()
            ...
            if CopyToClipboard then
                ...
                TA:Raw(
                    TA.LOG.OUTPUT,
                    "|cFF4AFF7A[ToonAge]|r Talent string copied! Open Talents (N) → Import → Paste."
                )
            end
