-- ToonAge/Modules/Progression/Delves.lua
...
function Delves:Render(content, sidebar)
    ...
    if tier.tier >= 5 then
        ...
        bTip:SetText(
            "Complete 4 Bountiful Delves this week for Great Vault credit.  Brann also earns bonus XP in Bountiful runs."
        )
    ...
    local tips = {
        {
            mode = "HEALER",
            color = { 0.29, 1.00, 0.48 },
            tip = "Sustains you via healing potions and salves. Best for new tiers and solo DPS.",
        },
        {
            mode = "DPS",
            color = { 1.00, 0.70, 0.20 },
            tip = "Deals bonus damage alongside you. Best when content is trivial or you have a healer.",
        },
        {
            mode = "TANK",
            color = { 0.29, 0.65, 1.00 },
            tip = "Intercepts and holds enemies. Rarely optimal but useful for swarm mechanics.",
        },
    }
...
function Delves:RenderSidebar(sidebar, mode, mc, tier, tc, iLvl)
    ...
    bountifulLbl:SetTextColor(
        tier.bountiful and 0.29 or 0.40,
        tier.bountiful and 1.00 or 0.40,
        tier.bountiful and 0.38 or 0.40,
        1
    )
    ...
    for _, t in ipairs(TIERS) do
        ...
        row:SetTextColor(
            ...
            current and 1 or (done and tcc[3] or 0.30),
            1
        )
