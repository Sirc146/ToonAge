-- ToonAge/Modules/DungeonGuide.lua
...
local function GetPlayerRole()
    ...
    if not ok or not role then return "DAMAGER" end
    return role
end
...
local function CreateGuideFrame()
    if DungeonGuide.frame then return DungeonGuide.frame end
...
function DungeonGuide:UpdateDisplay()
    ...
    if self.currentBossIndex < 1 then self.currentBossIndex = 1 end
    if self.currentBossIndex > bossCount then self.currentBossIndex = bossCount end
    ...
    f.roleLabel:SetText(string.format(
        "|cFF%02x%02x%02x%s|r",
