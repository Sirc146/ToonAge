...
function GI:ImportFromText(text)
    ...
    -- First real guide on a client that had none: bring the Guide tab and the
    -- Guide Tracker drawer online without a reload.
    if TA.UI and TA.UI.RebuildTabs then pcall(TA.UI.RebuildTabs, TA.UI) end
    if TA.Modern and TA.Modern.InitDrawer and not TA.Modern.drawer then
        TA.Modern._guideDisabled = nil
        pcall(TA.Modern.InitDrawer, TA.Modern)
    end
    return guide, nil
end
