...
-- ══════════════════════════════════════════════════════════════════════════════
-- ── STUB METHODS — ported as minimal implementations to prevent nil crashes ──
-- These are called by existing code but were never ported from the PTR build.
-- ══════════════════════════════════════════════════════════════════════════════

--- Rebuild the tracker window content (step text, objectives, buttons).
function QT:UpdateWindow()
    ...
    if win.lines then
        for _, line in ipairs(win.lines) do line:Hide() end
    end
    win.lines = win.lines or {}

    -- Show current step info
    local guide = TA.Guides and TA.Guides[self.guideID]
    if not guide then
        -- No guide active — show a placeholder
        if not win.placeholder then
            win.placeholder = win:CreateFontString(nil, "OVERLAY", "GameFontNormal")
            win.placeholder:SetPoint("CENTER")
            win.placeholder:SetText("No guide active.\nNo Mists of Pandaria leveling guides are bundled yet — /ta guides lists any you import.")
            win.placeholder:SetTextColor(0.6, 0.6, 0.6)
        end
        if win.placeholder then win.placeholder:Show() end
        return
    end

    if win.placeholder then win.placeholder:Hide() end

    local step = guide.steps and guide.steps[self.stepIdx]
...
--- Check if the player is close enough to auto-advance the current step.
function QT:CheckProximityAdvance()
    -- Stub: proximity auto-advance requires coord data in the guide.
    -- When guides have real coords this will check player distance to step coord.
end

--- Show a brief toast notification (e.g. "Step complete!").
function QT:ShowToast(msg)
    if msg then
        TA:Print(TA.LOG.INFO, "Guide", tostring(msg))
    end
end
...
-- ══════════════════════════════════════════════════════════════════════════════
-- ── STUB METHODS — minimal implementations to prevent nil crashes ─────────────
-- ══════════════════════════════════════════════════════════════════════════════
...
function QT:CheckProximityAdvance()
    -- Stub: requires real coords in guides to function
end
...
-- ── Render (Guide tab in the ToonAge panel) ───────────────────────────────────
function QT:Render(content, sidebar)
    ...
    if sidebar then
        ...
        title:SetText(guide and guide.title or "No Guide")

        if guide then
            local info = sidebar:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
            info:SetPoint("TOPLEFT", title, "BOTTOMLEFT", 0, -6)
            info:SetText(string.format("Step %d / %d", self.stepIdx or 1, #guide.steps))
    ...
    if not guide then
        ...
        msg:SetPoint("CENTER")
        msg:SetText("No guide active.\nNo Mists of Pandaria leveling guides are bundled yet — /ta guides lists any you import.")
        msg:SetTextColor(0.6, 0.6, 0.6)
        return
    end

    local yOff = -10
    local startIdx = math.max(1, (self.stepIdx or 1) - 1)
    local endIdx = math.min(#guide.steps, startIdx + 12)
    ...
    for i = startIdx, endIdx do
        ...
        if step then
            ...
            local prefix = isCurrent and "|cFF4AFF7A> " or "  "
            local suffix = isCurrent and "|r" or ""
            local typeTag = step.type and ("[" .. step.type .. "] ") or ""
            line:SetText(prefix .. typeTag .. (step.text or "") .. suffix)

            if not isCurrent then line:SetTextColor(0.6, 0.6, 0.6) end
            yOff = yOff - 20
        end
    end
end
