...
function Character:UpdateData()
    ...
    -- Check Hit/Exp caps (percent-based; boss caps at 90, same-level caps
    -- while leveling or in PvP mode). Casters combine hit + expertise.
    local hitPct, expPct = 0, 0
    for _, s in ipairs(secondaries) do
        if s.key == "HIT" then hitPct = SafeNum(s.pct) end
        if s.key == "EXP" then expPct = SafeNum(s.pct) end
    end
    local capLevel = (pvxMode == "pvp") and 0 or (UnitLevel("player") or 0)
    for _, s in ipairs(secondaries) do
