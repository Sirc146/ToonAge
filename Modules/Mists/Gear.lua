...
-- Armor proficiency in MoP (armor subclass IDs: 1 Cloth, 2 Leather, 3 Mail, 4 Plate):
--   Warrior/Paladin wear Mail until they learn Plate at 40; Hunter/Shaman wear
--   Leather until Mail at 40 (Wowhead MoP Classic spell 8737); Death Knights
--   start at 55 in Plate. Armor Specialization (+5% primary stat for wearing
--   only your class armor type) is learned at 50, so below 50 lighter armor
--   with better stats is a legitimate upgrade.
local ARMOR_SPEC_LEVEL = 50
local function MaxArmorType(cls, level)
    level = level or 90
    if cls == "DEATHKNIGHT" then return 4 end
    if cls == "WARRIOR" or cls == "PALADIN" then return level >= 40 and 4 or 3 end
    if cls == "HUNTER"  or cls == "SHAMAN"  then return level >= 40 and 3 or 2 end
    if cls == "ROGUE"   or cls == "DRUID"   or cls == "MONK" then return 2 end
    return 1  -- Cloth (Mage, Warlock, Priest)
end

local function ResolveArmorType()
    local _, cls = UnitClass("player")
    return MaxArmorType(cls, UnitLevel("player"))
end

-- true if an armor piece (itemClassID 4, body slot) of this subclass should be
-- considered for the player at their current level.
local function ArmorSubclassAllowed(subClassID, cls, level)
    if not subClassID or subClassID < 1 or subClassID > 4 then return true end
    if not cls then cls = select(2, UnitClass("player")) end
    level = level or UnitLevel("player") or 1
    local maxType = MaxArmorType(cls, level)
    if subClassID > maxType then return false end
    if level >= ARMOR_SPEC_LEVEL then return subClassID == maxType end
    return true
end

Gear.MaxArmorType = MaxArmorType
Gear.ArmorSubclassAllowed = ArmorSubclassAllowed
...
local function CalculateItemScore(itemLink, specID, mode)
    ...
    -- Callers such as AutoEquip pass only the link: default to the active
    -- spec and the character's PvE/PvP mode instead of returning 0.
    specID = specID or U.GetPlayerSpec()
    mode   = mode or (TA.charDB and TA.charDB.pvxMode) or "pve"
    ...
    local stats = {
        ...
        DODGE   = CleanProxyValue(rawStats["ITEM_MOD_DODGE_RATING_SHORT"]),
        PARRY   = CleanProxyValue(rawStats["ITEM_MOD_PARRY_RATING_SHORT"]),
        RESIL   = CleanProxyValue(rawStats["ITEM_MOD_RESILIENCE_RATING_SHORT"]),
        PVPPOWER = CleanProxyValue(rawStats["ITEM_MOD_PVP_POWER_SHORT"]),
    }
    ...
    for _, item in ipairs(Gear.bagCache.items) do
        ...
        if equipLoc then
            ...
            if isValid and item.itemClassID == 4 then
                ...
                if not isAccessory and not ArmorSubclassAllowed(item.itemSubClassID) then isValid = false end
            end
