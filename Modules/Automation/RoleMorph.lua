-- ToonAge/Modules/Automation/RoleMorph.lua
...
local function IsRangedSpec(specID)
    ...
    if data.primary == "INT" and data.role == "DAMAGER" then
        return true
    end
    ...
    -- Evoker DPS
    if specID == 1467 or specID == 1473 then
        return true
    end
    return false
end
...
function RM:CheckHeirlooms()
    ...
    if #deadSlots > 0 then
        ...
        for _, info in ipairs(deadSlots) do
            TA:Raw(
                TA.LOG.WARN,
                string.format("  |cFFFF4444✗|r %s (capped at level %d) — replace ASAP", info.name, info.cap)
            )
        end
    ...
    for i = 1, 40 do
        ...
        if auraData.spellId then
            ...
            for _, fid in ipairs(correctIDs) do
                if auraData.spellId == fid then
                    hasCorrectFlask = true
                end
            end
