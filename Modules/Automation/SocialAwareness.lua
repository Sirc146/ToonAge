-- ToonAge/Modules/Automation/SocialAwareness.lua
...
function Social:ScanNearbyPlayers()
    ...
    if not playerZone or playerZone == "" then
        return
    end
...
function Social:GetSuggestion()
    ...
    if #self.nearbyFriends == 1 then
        ...
        return string.format(
            "|cFF55CCFF👥 %s (%s, lvl %d) is in %s — party up?|r",
            f.name,
            f.source,
            f.level,
            f.zone
        ...
        return string.format(
            "|cFF55CCFF👥 %d friends/guildmates nearby in %s!|r",
            #self.nearbyFriends,
            GetRealZoneText()
        )
    end
