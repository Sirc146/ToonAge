-- ToonAge/Modules/Automation/AutoMount.lua
    ...
    if C_MountJournal and C_MountJournal.GetCollectedDragonridingMounts then
        ...
        if ok and mounts and #mounts > 0 then
            ...
            if C_MountJournal.GetMountInfoByID then
                local infoOk, name, spellID, icon, isActive, isUsable = pcall(C_MountJournal.GetMountInfoByID, mountID)
...
function AutoMount:Init()
    ...
    if TA.debug then
        ...
        TA:Raw(
            TA.LOG.INFO,
            "|cFFFFD100[TA]|r AutoMount module loaded. Enabled: "
                .. tostring(settings and settings.enabled)
                .. ", Delay: "
                .. tostring(settings and settings.delay)
                .. "s"
        )
    end
end
