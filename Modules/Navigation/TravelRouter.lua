-- ToonAge/Modules/Navigation/TravelRouter.lua
...
function TR:FindRoute(fromZone, toZone)
    if not fromZone or not toZone or fromZone == toZone then
        return nil
    end
    if fromZone == 0 or toZone == 0 then
        return nil
    end
    ...
    if hearthMapID and hearthMapID == toZone then
        ...
        local ready = not cdInfo or cdInfo.duration == 0 or ((cdInfo.startTime + cdInfo.duration) - GetTime()) <= 0
        if ready then
            ...
            return {
                method = "hearth",
                label = string.format("Hearth (%d:%02d cd)", math.floor(remaining / 60), remaining % 60),
                hops = 1,
            }
        end
    end
