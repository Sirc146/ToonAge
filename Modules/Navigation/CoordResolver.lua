-- ToonAge/Modules/Navigation/CoordResolver.lua
...
local function QueryAPR(questID, stepType)
    if not APR or not APR.RouteQuestStepList then
        return nil
    end
            ...
            if steps then
                for _, step in ipairs(steps) do
                    ...
                    if coord and coord.x and coord.y then
                        ...
                        local questIDs = {}
                        if step.PickUp then
                            for _, qid in ipairs(step.PickUp) do
                                questIDs[qid] = "pickup"
                            end
                        end
                        if step.Done then
                            for _, qid in ipairs(step.Done) do
                                questIDs[qid] = "turnin"
                            end
                        end
                        if step.Qpart then
                            for qid, _ in pairs(step.Qpart) do
                                questIDs[qid] = "objective"
                            end
                        end
...
local function QueryTaskQuest(questID)
    if not C_TaskQuest or not C_TaskQuest.GetQuestLocation then
        return nil
    end
...
CR.SlashCommands = {
    coord = function(self)
        ...
        if C_QuestLog and C_QuestLog.GetNextWaypointForMap then
            table.insert(sources, "C_QuestLog POI")
        end
        if APR and APR.RouteQuestStepList then
            local routeCount = 0
            for _ in pairs(APR.RouteQuestStepList) do
                routeCount = routeCount + 1
            end
            table.insert(sources, "APR (" .. routeCount .. " routes)")
        end
        if C_TaskQuest then
            table.insert(sources, "C_TaskQuest")
        end
        if C_SuperTrack then
            table.insert(sources, "C_SuperTrack")
        end
