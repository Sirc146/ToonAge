-- ToonAge/Modules/Navigation/Arrow.lua
...
local function GetTargetStep()
    ...
    if not (QT and QT.guideID and QT.stepIdx) then
        return nil
    end
...
function Arrow:InitFrame()
    ...
    f:SetScript("OnUpdate", function(_, elapsed)
        ...
        if self.throttle < UPDATE_HZ then
            return
        end
        self.throttle = 0
        self:Tick(f)
...
function Arrow:Tick(f)
    ...
    if coordMap ~= 0 and coordMap ~= currentMap then
        ...
        for _ = 1, 5 do -- max 5 levels of parent traversal
            ...
            if mapInfo.parentMapID == coordMap then
                isSameArea = true
                break
            end
        ...
        if not isSameArea then
            ...
            for _ = 1, 5 do
                ...
                if mapInfo.parentMapID == currentMap then
                    isSameArea = true
                    break
                end
                if mapInfo.parentMapID and mapInfo.parentMapID > 0 then
    ...
    local r, g, b = ColorGradient(
        ...
        0.90,
        0.20,
        0.15, -- red (facing away)
        1.00,
        0.80,
        0.10, -- yellow (sideways)
        0.20,
        0.92,
        0.40 -- green (facing toward)
    )
...
function Arrow:FindNearestFlightMaster(currentMap)
    local TR = TA:GetModule("TravelRouter")
    if not TR or not TR.knownFlightPaths then
        return nil, nil, nil
    end
    ...
    if not bestX and C_TaxiMap and C_TaxiMap.GetTaxiNodesForMap then
        ...
        if nodes then
            for _, node in ipairs(nodes) do
                if
                    node.position
                    and (node.state == Enum.FlightPathState.Current or node.state == Enum.FlightPathState.Reachable)
                then
...
function Arrow:ParseWayCommand(args)
    if not args or args == "" then
        ...
        if self.manualWaypoint then
            ...
            TA:Raw(
                TA.LOG.OUTPUT,
                string.format("  Current: map %d — %.1f, %.1f (%s)", wp.map, wp.x * 100, wp.y * 100, wp.title or "")
            )
        end
        return
    end
    ...
    TA:Raw(
        TA.LOG.OUTPUT,
        string.format(
            "|cFFFFD100[TA Arrow]|r Waypoint set: |cFF4AFF7A%.1f, %.1f|r%s%s",
            xRaw,
            yRaw,
            mapStr,
            desc and (" — " .. desc) or ""
