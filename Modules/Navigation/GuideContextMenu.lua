-- ToonAge/Modules/Navigation/GuideContextMenu.lua
...
local function SwitchToGuideForQuest(questID, skipPopup)
    ...
    if not GI or not GI.FindGuideForQuest then
        return
    end
...
StaticPopupDialogs[POPUP_NAME] = {
    ...
    OnShow = function(self)
        ...
        self.text:SetFormattedText(StaticPopupDialogs[POPUP_NAME].text, guideName)
    end,
...
local function SetupMenuHooks()
    ...
    if TA.debug then
        TA:Raw(
            TA.LOG.OUTPUT,
            "|cFFFFD100[TA GuideContextMenu]|r Menu hooks registered."
    ...
    local function TryHookDropDown()
        ...
        if ObjectiveTrackerBlockDropDown then
            ...
            hooksecurefunc("UIDropDownMenu_Initialize", function(frame, initFunc, ...)
                if frame ~= ObjectiveTrackerBlockDropDown then
                    return
                end
...
function GCM:PrintGuideFoundMessage(questID, guideID, guideName)
    ...
    TA:Raw(
        TA.LOG.OUTPUT,
        string.format("|cFFFFD100[ToonAge]|r Quest #%d belongs to |cFFFFFFFF%s|r — %s", questID, guideName, hyperlink)
    )
end
