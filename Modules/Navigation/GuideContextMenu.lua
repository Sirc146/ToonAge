...
local function SwitchToGuideForQuest(questID, skipPopup)
    ...
    -- Pass guideName as text_arg1 — the standard StaticPopup substitution
    -- mechanism for a "%s" in the dialog's text — since Blizzard's own Init
    -- formats the text immediately using this argument, before OnShow runs.
    StaticPopup_Show(POPUP_NAME, guideName)
end
...
StaticPopupDialogs[POPUP_NAME] = {
    ...
    -- NOTE: no OnShow text-injection here. Blizzard's own StaticPopup/GameDialog
    -- Init() runs BEFORE OnShow and unconditionally calls self.text:SetFormattedText
    -- using whatever text_arg1/text_arg2 were passed to StaticPopup_Show(). Since
    -- this dialog's `text` string contains a "%s", calling StaticPopup_Show(POPUP_NAME)
    -- with no extra args left that first substitution nil, and Blizzard's own Init
    -- crashed on it ("bad argument #2 to 'SetFormattedText' (string expected, got nil)")
    -- before this OnShow handler ever got a chance to run and fix it up. The correct
    -- fix is to pass the guide name as text_arg1 to StaticPopup_Show itself (see
    -- SwitchToGuideForQuest below) so Blizzard's own Init formats it correctly.
