-- ToonAge/Modules/Navigation/GuideBrowser.lua
...
function GB:OrganizeGuides()
    ...
    for id, guide in pairs(TA.Guides or {}) do
        ...
        for _, exp in ipairs(EXPANSION_ORDER) do
            if
                id:lower():find(exp:lower())
                or title:lower():find(exp:lower())
                or title:find(EXPANSION_LABELS[exp] or "")
            then
                expansion = exp
                break
            end
        ...
        if expansion == "Other" then
            ...
            if minLv >= 80 then
                expansion = "Midnight"
            elseif minLv >= 70 then
                expansion = "TheWarWithin"
            elseif minLv >= 60 then
                expansion = "Dragonflight"
            elseif minLv >= 50 then
                expansion = "Shadowlands"
            elseif minLv >= 1 and (guide.maxLevel or 999) <= 10 then
                expansion = "Classic"
            end
        end
    ...
    for _, exp in ipairs(EXPANSION_ORDER) do
        ...
        if guides and #guides > 0 then
            ...
            for _, g in ipairs(guides) do
                ...
                if #displayTitle > 35 then
                    displayTitle = displayTitle:sub(1, 32) .. "..."
                end
                ...
                row:SetScript("OnClick", function()
                    if QT then
                        ...
                        TA:Raw(
                            TA.LOG.OUTPUT,
                            string.format("|cFFFFD100[ToonAge]|r Guide switched to: |cFFFFFFFF%s|r", g.title)
                        )
                        GB:RefreshBrowser()
                    end
                end)
    ...
    switchto = function(self, msg)
        ...
        if guideID then
            ...
            if QT then
                ...
                TA:Raw(
                    TA.LOG.OUTPUT,
                    string.format("|cFFFFD100[ToonAge]|r Switched to: |cFFFFFFFF%s|r", guide and guide.title or guideID)
                )
            end
        else
