...
function QT:HandleAutoQuest(event)
    ...
    -- Zygor handles accept/turn-in/rewards; doing it twice picks rewards twice.
    if U.DeferToZygor and U.DeferToZygor() then return end
    ...
    -- sandboxing (gossip/quest-detail context can be tainted). :match() on such
    -- a value throws — pcall catches it instead of erroring out auto-quest handling.
    local okMatch, npcIDStr = pcall(string.match, targetGUID, "Creature%-0%-%d+%-%d+%-%d+%-(%d+)")
    local npcID = okMatch and npcIDStr and tonumber(npcIDStr) or nil
    ...
    elseif event == "QUEST_COMPLETE" then
        ...
        for i = 1, numChoices do
            ...
            if link then
                ...
                -- Use Gear module's stat scoring if available. (This used to
                -- call a ScoreItem method that doesn't exist, so rewards
                -- were always picked by item level alone.)
                if GearMod and GearMod.CalculateItemScore then
                    local specID = U.GetPlayerSpec and U.GetPlayerSpec()
                    local ok, s = pcall(GearMod.CalculateItemScore, link, specID,
                        (TA.charDB and TA.charDB.pvxMode) or "pve")
                    if ok and type(s) == "number" and s > 0 then score = s end
                end
