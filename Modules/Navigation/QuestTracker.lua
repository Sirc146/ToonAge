-- ToonAge/Modules/Navigation/QuestTracker.lua
...
function QT:GetQuestStatus(questID)
    ...
    if IsReadyForTurnIn(questID) then
        return "turnin"
    end
...
function QT:IsStepApplicable(step)
    ...
    if step.class then
        ...
        if pClass ~= step.class then
            return false
        end
    end
    if step.spec then
        local specID = GetSpecializationInfo(GetSpecialization())
        if specID ~= step.spec then
            return false
        end
    end
    if step.minLevel then
        local lvl = UnitLevel("player") or 1
        if lvl < step.minLevel then
            return false
        end
    end
    return true
...
function QT:IsStepComplete(step)
    ...
    if sType == "objective" then
        if step.questID and step.objectiveIndex then
            return IsObjectiveFinished(step.questID, step.objectiveIndex) or IsComplete(step.questID)
        end
        return false
    end
    ...
    if sType == "waypoint" then
        if IsPlayerFlying() then
            return true
        end -- auto-skip waypoints while flying
    ...
    if sType == "quest" then
        if step.questID then
            if IsComplete(step.questID) then
                return true
            end
            -- Smart Phrase Parsing: accept/speak/talk steps finish when quest is in log
            local text = (step.text or ""):lower()
            local isAcceptStep = text:match("^accept") or text:match("^speak") or text:match("^talk")
            if isAcceptStep and IsInLog(step.questID) then
                return true
            end
        end
        return false
...
function QT:IsPrerequisiteMet(step)
    if not step.pre and not step.preOr then
        return true
    end
...
function QT:ShouldShowStep(step, stepIdx)
    ...
    if self.skippedSteps[stepIdx] then
        return false
    end
    ...
    -- Prerequisite chain
    if not self:IsPrerequisiteMet(step) then
        return false
    end

    -- Rank filter
    if not self:PassesRankFilter(step) then
        return false
    end

    -- Active condition (only show if specific quest is in log)
    if not self:IsActiveConditionMet(step) then
        return false
    end
...
local function MapZoneDistance(playerMapID, guideZone)
    if not guideZone or guideZone == 0 or not playerMapID then
        return nil
    end
    if playerMapID == guideZone then
        return 0
    end
    local info, depth = C_Map.GetMapInfo(playerMapID), 0
    while info and info.parentMapID and info.parentMapID > 0 and depth < 12 do
        depth = depth + 1
        if info.parentMapID == guideZone then
            return depth
        end
...
function QT:AutoSelectGuide()
    if self:SmartMatchGuideFromLog() then
        return
    end
...
function QT:Diagnose()
    local p = function(msg)
        TA:Raw(TA.LOG.OUTPUT, "|cFFFFD100[TA Tracker]|r " .. msg)
    end
    ...
    p(string.format("Player: level %d  |  map %s (ID %s)", level, mapName, tostring(currentMap)))
    ...
    p(string.format("Quest log: %d active quest(s)", #(function()
        local t = {}
    ...
    for _, entry in ipairs(list) do
        ...
        local flags = {}
        if levelMatch then
            flags[#flags + 1] = "|cFF4AFF7Alevel✓|r"
        else
            flags[#flags + 1] = "|cFFFF4444level✗|r"
        end
        if zoneMatch then
            flags[#flags + 1] = "|cFF4AFF7Azone✓|r"
        else
            flags[#flags + 1] = "|cFF888780zone✗|r"
        end
        if matches > 0 then
            flags[#flags + 1] = string.format("|cFFFFD100%d quest match(es)|r", matches)
        end

        p(
            string.format(
                "  [%s] '%s'  lvl %d-%d  zone=%d  %s",
                entry.id,
                g.title,
                gMin,
                gMax,
    ...
    if self.guideID then
        p(
            "Active guide: '"
                .. (TA.Guides[self.guideID] and TA.Guides[self.guideID].title or self.guideID)
                .. "'  step "
                .. self.stepIdx
        )
    else
...
function QT:HandleAutoQuest(event)
    if not (TA.charDB and TA.charDB.tracker and TA.charDB.tracker.autoQuest) then
        return
    end
    ...
    if npcID and NPC_BLOCKLIST[npcID] then
        return
    end
    ...
    if event == "QUEST_DETAIL" then
        ...
        if questID and QUEST_BLOCKLIST[questID] then
            return
        end
        ...
        if TA.charDB.tracker.autoQuestGuideOnly and questID then
            ...
            if not expectedIDs[questID] then
                return
            end
        end
        ...
        local offeredByPlayer = (QuestIsFromAreaTrigger and not QuestIsFromAreaTrigger()) and UnitIsPlayer("questnpc")
        if offeredByPlayer then
            -- Only auto-accept shares from friends/guild
            local name = UnitName("questnpc")
            local isFriend = name
                and (
                    C_FriendList.IsFriend(name)
                    or (
                        C_BattleNet
    ...
    elseif event == "QUEST_PROGRESS" then
        ...
        if QuestProgressRequiresGold and QuestProgressRequiresGold() then
            return
        end
        ...
        for i = 1, numItems do
            ...
            if numRequired and numRequired > 0 then
                ...
                if link then
                    ...
                    if itemType == "Tradeskill" or itemSubType == "Reagent" then
                        return
                    end
                end
            end
...
function QT:UpdateBlizzardTrackerVisibility()
    if not (TA.charDB and TA.charDB.tracker) then
        return
    end
    local shouldHide = TA.charDB.tracker.replaceBlizzTracker and self.window and self.window:IsVisible()
...
function QT:Init()
    ...
    if t.autoQuest == nil then
        t.autoQuest = false
    end
    if t.replaceBlizzTracker == nil then
        t.replaceBlizzTracker = false
    end
    if t.cutsceneSkip == nil then
        t.cutsceneSkip = false
    end
    if t.autoEquip == nil then
        t.autoEquip = false
    end
    if t.autoQuestGuideOnly == nil then
        t.autoQuestGuideOnly = false
    end
    if t.rank == nil then
        t.rank = 2
    end -- 1=speed, 2=normal, 3=completionist

    -- Guide display settings
    if t.showAvailableQuests == nil then
        t.showAvailableQuests = true
    end
    if t.smallMapPins == nil then
        t.smallMapPins = false
    end
    if t.showCategoryGrid == nil then
        t.showCategoryGrid = false
    end
    if t.showCategoryHeaders == nil then
        t.showCategoryHeaders = true
    end
    if t.groupCompleted == nil then
        t.groupCompleted = true
    end
    if t.groupIgnored == nil then
        t.groupIgnored = true
    end
    if t.showQuestChainTooltip == nil then
        t.showQuestChainTooltip = true
    end
    if t.spoilerFree == nil then
        t.spoilerFree = false
    end
    if t.useTomTomWaypoints == nil then
        t.useTomTomWaypoints = false
    end
    if t.accountBound == nil then
        t.accountBound = false
    end
...
local function MakeCheckbox(parent, x, y, label, dbKey, onChange)
    ...
    chk:SetPoint("CENTER")
    chk:SetSize(8, 8)
    if TA.charDB.tracker[dbKey] then
        chk:Show()
    else
        chk:Hide()
    end
    ...
    cb:SetScript("OnClick", function()
        ...
        if onChange then
            onChange(TA.charDB.tracker[dbKey])
        end
    end)
    return cb
end
...
function QT:InitWindow()
    ...
    local xBtn = MakeBtn(titleBar, 22, 22, "x", function()
        self:ToggleWindow()
    end)
    ...
    local prevBtn = MakeBtn(win, 24, 20, "<", function()
        self:CycleGuide(-1)
    end)
    prevBtn:SetPoint("TOPLEFT", win, "TOPLEFT", PAD, -34)

    local nextBtn = MakeBtn(win, 24, 20, ">", function()
        self:CycleGuide(1)
    end)
    ...
    win:SetScript("OnUpdate", function(_, elapsed)
        ...
        if self.statusThrottle < STATUS_UPDATE_HZ then
            return
        end
    ...
    qib:SetScript("OnLeave", function()
        GameTooltip:Hide()
    end)
...
function QT:UpdateQuestItemButton()
    ...
    if not (win and win.questItemBtn) then
        return
    end
...
function QT:UpdateWindow()
    ...
    if not guide then
        ...
        local loaded = 0
        for _ in pairs(TA.Guides or {}) do
            loaded = loaded + 1
        end
        ...
        if not trackedTitle then
            ...
            for i = 1, numEntries do
                ...
                if
                    ...
                    and info.questID
                    and not C_QuestLog.IsQuestFlaggedCompleted(info.questID)
                then
                    trackedQuestID = info.questID
                    trackedTitle = info.title
                    trackedObjectives = C_QuestLog.GetQuestObjectives(info.questID)
    ...
    if not self:IsStepApplicable(step) then
        ...
        if step.questID then
            local liveTitle = C_QuestLog.GetTitleForQuestID and C_QuestLog.GetTitleForQuestID(step.questID)
        ...
        if step.questID and IsInLog(step.questID) then
            local objectives = C_QuestLog.GetQuestObjectives and C_QuestLog.GetQuestObjectives(step.questID)
        ...
        local function Assemble(shownObjCount)
            local lines = {}
            for _, l in ipairs(headLines) do
                lines[#lines + 1] = l
            end
            for i = 1, shownObjCount do
                lines[#lines + 1] = objLines[i]
            end
            ...
            for _, l in ipairs(tailLines) do
                lines[#lines + 1] = l
            end
    ...
    if MapPins and MapPins.Refresh then
        MapPins:Refresh()
    end
end
...
function QT:RenderStatusLine()
    ...
    if step and step.coord and step.type ~= "text" then
        ...
        if Arrow and Arrow.GetEffectiveCoord then
            ...
            local currentMap = C_Map.GetBestMapForUnit("player")
            if
                not (coordMap == 0 and cx == 0 and cy == 0)
                and currentMap
                and (coordMap == 0 or coordMap == currentMap)
            then
                local pos = C_Map.GetPlayerMapPosition(currentMap, "player")
                if pos then
                    hasLiveTarget = true
                    ...
                    distStr = "  |cFF888780" .. U.FormatDistance(yards) .. "  " .. U.FormatETA(yards, speed) .. "|r"
                end
            end
        end
    end
...
function QT:ToggleWindow()
    ...
    if self.window:IsVisible() then
        ...
        if TA.charDB then
            TA.charDB.tracker.visible = false
        end
        self:UpdateBlizzardTrackerVisibility()
    else
        self.window:Show()
        ...
        if self.guideID and TA.Guides and TA.Guides[self.guideID] then
            ...
            TA:Raw(
                TA.LOG.INFO,
                string.format("|cFFFFD100[TA Tracker]|r '%s' — step %d/%d", g.title, self.stepIdx, #g.steps)
            )
        end
    end
end
...
function QT:AnalyzeQuestLog()
    ...
    for id, guide in pairs(TA.Guides or {}) do
        if guide.expansion == "midnight" or (guide.minLevel and guide.minLevel >= 80) then
            for _, step in ipairs(guide.steps or {}) do
                if step.questID then
                    midnightQuestIDs[step.questID] = true
                end
            end
        end
    end
    ...
    results.totalQuests = #results.currentExpansion
        + #results.oldExpansion
        + #results.lowLevel
        + #results.guideRelated
        + #results.unknown
...
function QT:ShowQuestLogCleanup()
    ...
    local dropList = {}
    for _, q in ipairs(analysis.lowLevel) do
        table.insert(dropList, q)
    end
    for _, q in ipairs(analysis.oldExpansion) do
        table.insert(dropList, q)
    end
...
function QT:AnalyzeQuestProgress()
        ...
        if info and not info.isHeader and info.questID then
            ...
            if readyForTurnIn then
                ...
                if objectives and #objectives > 0 then
                    ...
                    table.insert(
                        quests.inProgress,
                        { questID = questID, title = title, pct = 0, doneObj = 0, totalObj = 0 }
                    )
                end
            end
        end
    ...
    table.sort(quests.inProgress, function(a, b)
        return a.pct > b.pct
    end)
    table.sort(quests.almostDone, function(a, b)
        return a.pct > b.pct
    end)
...
function QT:ShowQuestLogAdvisor()
    ...
    -- Count total
    local total = #progress.readyToTurnIn + #progress.almostDone + #progress.inProgress + #progress.notStarted
    ...
    if freeableNow > 0 then
        TA:Raw(
            TA.LOG.OUTPUT,
            string.format(
                "|cFF4AFF7A→ You can free %d slot(s) immediately by turning in completed quests.|r",
                freeableNow
            )
        )
    elseif freeableSoon > 0 then
        TA:Raw(
            TA.LOG.OUTPUT,
            string.format(
                "|cFFFFD100→ Finish %d almost-done quest(s) to free up slots without dropping anything.|r",
                freeableSoon
            )
        )
    else
        TA:Raw(
            TA.LOG.OUTPUT,
            "|cFFFF9A1A→ No quests are close to completion. Consider finishing the highest-% ones first,|r"
        )
...
function QT:ShowDropUnrelatedPopup()
    ...
    if #unrelated == 0 then
        TA:Raw(
            TA.LOG.OUTPUT,
            "|cFF4AFF7A[ToonAge]|r All quests in your log are part of the active guide. Nothing to drop."
        )
        return
    end
...
function QT:ClassifyGuideExpansion(guide)
    ...
    if minLvl >= 60 and maxLvl <= 70 then
        return "df"
    end
    if minLvl >= 50 and maxLvl <= 60 then
        return "sl"
    end
    if minLvl >= 45 and maxLvl <= 50 then
        return "bfa"
    end
    if maxLvl <= 20 then
        return "starter"
...
function QT:RenderMiddlePanel(content)
    ...
    local function Track(f)
        table.insert(self.middlePanelFrames, f)
        return f
    end
    ...
    for id, guide in pairs(TA.Guides or {}) do
        ...
        if guideExp == selectedExp then
            ...
            table.insert(zoneGuides, {
                id = id,
                title = guide.title or id,
                minLevel = guide.minLevel or 1,
                maxLevel = guide.maxLevel or 99,
                total = total,
                completed = completed,
                pct = pct,
            })
    ...
    table.sort(zoneGuides, function(a, b)
    ...
    for _, zg in ipairs(zoneGuides) do
        ...
        followBtn:SetScript("OnEnter", function(f)
            f:SetBackdropColor(fBg[1] + 0.06, fBg[2] + 0.06, fBg[3] + 0.03, 1)
        end)
        followBtn:SetScript("OnLeave", function(f)
            f:SetBackdropColor(fBg[1], fBg[2], fBg[3], 1)
        end)
        ...
        if zg.pct < 100 and zg.pct > 0 then
            ...
            mBtn:SetScript("OnEnter", function(f)
                f:SetBackdropColor(0.12, 0.07, 0.03, 1)
            end)
            mBtn:SetScript("OnLeave", function(f)
                f:SetBackdropColor(0.08, 0.04, 0.02, 1)
            end)
        end
...
function QT:ShowToast(message, duration)
    ...
    if toast._fadeTimer then
        toast._fadeTimer:Cancel()
...
function QT:ShowTrackerMenuModern(anchor)
    MenuUtil.CreateContextMenu(anchor, function(ownerRegion, rootDescription)
        ...
        rootDescription:CreateCheckbox("Auto-Accept/Turn-In Quests", function()
        ...
        rootDescription:CreateCheckbox("Auto-Equip Upgrades", function()
...
function QT:ShowTrackerMenuLegacy(anchor)
    ...
    if menu._buttons then
        for _, btn in ipairs(menu._buttons) do
            btn:Hide()
        end
    end
    menu._buttons = {}
    ...
    local function AddButton(text, onClick)
        ...
        btn:SetScript("OnEnter", function()
            lbl:SetTextColor(1, 0.95, 0.75, 1)
        end)
    ...
    AddButton("|cFF4AE0FFOpen ToonAge Panel|r", function()
        TA:ToggleUI()
    end)
    ...
    AddButton("Re-Sync Position", function()
        self:FastForward(false)
    end)
    ...
    AddButton(
        (TA:GetModule("Arrow") and TA:GetModule("Arrow").frame and TA:GetModule("Arrow").frame:IsVisible())
                and "Hide Arrow"
            or "Show Arrow",
        function()
            local Arrow = TA:GetModule("Arrow")
    ...
    AddButton(
        (TA:GetModule("NavHud") and TA:GetModule("NavHud").frame and TA:GetModule("NavHud").frame:IsShown())
                and "Hide NavHud"
            or "Show NavHud",
        function()
            local NavHud = TA:GetModule("NavHud")
    ...
    AddButton("Hide Tracker", function()
        self:ToggleWindow()
    end)
...
QT.SlashCommands = {
    tracker = function(self)
        self:ToggleWindow()
    end,

    drop = function(self)
        self:ShowDropUnrelatedPopup()
    end,
    ...
    autoselect = function(self)
        ...
        if self.guideID and TA.Guides and TA.Guides[self.guideID] then
            ...
            TA:Raw(
                TA.LOG.INFO,
                string.format(
                    "|cFFFFD100[TA Tracker]|r Auto-selected: '%s' — step %d/%d",
                    g.title,
                    self.stepIdx,
                    #g.steps
                )
            ...
            TA:Raw(
                TA.LOG.INFO,
                "|cFFFFD100[TA Tracker]|r No guide matched. Run |cFFFFD100/ta diag|r to see what the tracker sees."
            )
        end
    end,
...
function QT:CheckProximityAdvance()
    ...
    local Arrow = TA:GetModule("Arrow")
    if not Arrow or not Arrow.GetEffectiveCoord then
        return
    end
...
function QT:UpdateDrawer()
    local M = TA.Modern
    if not M or not M.drawer or not M.drawer:IsVisible() then
        return
    end
    ...
    if step.questID then
        ...
        if qs == "complete" then
            statusText = "Complete"
            statusClr = M.CLR_TEXT_SUCCESS
        end
        if qs == "turnin" then
            statusText = "Ready to Turn In"
            statusClr = M.CLR_TEXT_WARNING
        end
        if qs == "inprogress" then
            statusText = "In Progress"
            statusClr = M.CLR_TEXT_SECONDARY
        end
        if qs == "available" then
    ...
    if Arrow and Arrow.GetEffectiveCoord and step.coord and step.type ~= "text" then
        ...
        if currentMap and not (coordMap == 0 and cx == 0 and cy == 0) then
            ...
            if pos then
                ...
                local speed = (TM and TM:GetSpeed()) or 7
                local distF =
                    M:CreateCaption(content, TA.Utils.FormatDistance(yards) .. "  " .. TA.Utils.FormatETA(yards, speed))
