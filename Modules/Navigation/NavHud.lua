-- ToonAge/Modules/Navigation/NavHud.lua
...
function NavHud:Tick()
    ...
    if self.frame.coordText then
        self.frame.coordText:SetShown(showCoords)
    end
    if self.frame.distText then
        self.frame.distText:SetShown(showDist)
    end
    if self.frame.stepText then
        self.frame.stepText:SetShown(showStep)
    end
...
function NavHud:ApplySettings()
    ...
    if self.frame.ring then
        self.frame.ring:SetShown(showRing)
    end
    if self.frame.rangeCircle then
        self.frame.rangeCircle:SetShown(showRing)
    end
end
...
local function PrintHudSettings()
    ...
    TA:Raw(
        TA.LOG.OUTPUT,
        "  |cFFFFD100showDistance|r = " .. tostring(GetSetting("showDistance")) .. "  (yards to waypoint)"
    )
    TA:Raw(
        TA.LOG.OUTPUT,
        "  |cFFFFD100showStepText|r = " .. tostring(GetSetting("showStepText")) .. "  (step description)"
    )
    TA:Raw(TA.LOG.OUTPUT, "  |cFFFFD100showRing|r = " .. tostring(GetSetting("showRing")) .. "  (proximity circle)")
    TA:Raw(
        TA.LOG.OUTPUT,
        "  |cFFFFD100showGatherDots|r = " .. tostring(GetSetting("showGatherDots")) .. "  (gather nodes)"
    )
