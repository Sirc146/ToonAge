-- ToonAge/Modules/Navigation/GuideParser.lua
...
local function ValidateCoord(id, stepN, coord)
    if type(coord) ~= "table" then
        LogError(id, stepN, "coord must be a table")
        return false
    end
    if type(coord.map) ~= "number" then
        LogError(id, stepN, "coord.map must be a number")
        return false
    end
    if type(coord.x) ~= "number" or coord.x < 0 or coord.x > 1 then
        LogError(id, stepN, "coord.x must be a number in [0,1]")
        return false
    end
    if type(coord.y) ~= "number" or coord.y < 0 or coord.y > 1 then
        LogError(id, stepN, "coord.y must be a number in [0,1]")
        return false
    end
    return true
end
...
local function ValidateStep(id, n, step)
    if type(step) ~= "table" then
        LogError(id, n, "step must be a table")
        return false
    end
    local ok = true
...
function GP:Init()
    for _, e in ipairs(_errors) do
        local where = e.stepN and ("Guide '" .. e.id .. "' step " .. e.stepN) or ("Guide '" .. e.id .. "'")
    ...
    for _, s in ipairs(_summary) do
        if s.valid then
            ...
            if TA.debug then
                TA:Raw(
                    TA.LOG.OUTPUT,
                    string.format("|cFFFFD100[TA]|r Guide '|cFFFFFFFF%s|r' |cFF1EFF00OK|r (%d steps)", s.title, s.count)
                )
            end
        else
            TA:Raw(
                TA.LOG.OUTPUT,
                string.format(
                    "|cFFFFD100[TA]|r Guide '|cFFFFFFFF%s|r' |cFFFF4444INVALID|r (%d error(s))",
                    s.title,
                    s.errCount
                )
            )
    ...
    if #_summary == 0 then
        if TA.debug then
            TA:Raw(
                TA.LOG.OUTPUT,
                "|cFFFFD100[TA]|r No guide files found — add *.lua to Data/Guides/ and list in the .toc"
            )
        end
    elseif loadedCount > 0 then
        TA:Raw(
            TA.LOG.INFO,
            string.format(
                "|cFFFFD100[TA]|r %d guide(s) loaded. Type |cFFFFD100/ta tracker|r to open the tracker.",
                loadedCount
            )
        )
    end
...
function GP:IsStepApplicable(step)
    ...
    if step.faction then
        ...
        if step.faction ~= playerFaction then
            return false
        end
    end
    ...
    if step.class then
        ...
        if step.class ~= playerClass then
            return false
        end
    end
    ...
    if step.minLevel then
        ...
        if playerLevel < step.minLevel then
            return false
        end
    end
...
function GP:IsGuideApplicable(guide)
    ...
    if guide.faction then
        local playerFaction = UnitFactionGroup("player")
        if guide.faction ~= playerFaction then
            return false
        end
    end
    local playerLevel = UnitLevel("player")
...
function GP:DumpGuides()
    ...
    for id, g in pairs(TA.Guides) do
        ...
        TA:Raw(
            TA.LOG.OUTPUT,
            string.format(
                '|cFFFFD100[TA]|r  [%s] "%s"  lvl %d-%d  (%d steps)%s',
                id,
                g.title,
                g.minLevel or 1,
                g.maxLevel or 999,
