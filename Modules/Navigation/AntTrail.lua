-- ToonAge/Modules/Navigation/AntTrail.lua
...
function AntTrail:BuildTrail()
    ...
    if not QT or not CR or not QT.guideID then
        return {}
    end
...
function AntTrail:UpdateOnNavHud()
    ...
    for i = dotIdx + 1, #self.trailDots do
        if self.trailDots[i] then
            self.trailDots[i]:Hide()
        end
    end
    for i = dotIdx, #self.trailLines do
        if self.trailLines[i] then
            self.trailLines[i]:Hide()
        end
    end
end
...
function AntTrail:GetDot(index, parent)
    if self.trailDots[index] then
        return self.trailDots[index]
    end
...
function AntTrail:GetLine(index, parent)
    if self.trailLines[index] then
        return self.trailLines[index]
    end
...
function AntTrail:HideAll()
    ...
    for _, line in ipairs(self.trailLines) do
        line:Hide()
    end
end
...
AntTrail.SlashCommands = {
    trail = function(self)
        ...
        if #wp == 0 then
            TA:Raw(
                TA.LOG.OUTPUT,
                "|cFFFFD100[ToonAge Trail]|r No trail waypoints (no active guide or no coords available)."
            )
