-- ToonAge/Modules/GuideImporter.lua
...
function GI:ImportFromQuestLineAPI()
    if not C_QuestLine or not C_QuestLine.GetAvailableQuestLines then return 0 end
    ...
    if not questLines then return 0 end

    for _, qlInfo in pairs(questLines) do
        local guideID = "ql_" .. qlInfo.questLineID
        if TA.Guides[guideID] then break end -- already imported
        ...
        if quests and #quests > 0 then
            ...
            for _, questID in ipairs(quests) do
                table.insert(steps, {
                    type = "pickup", questID = questID, text = "Accept quest",
                    coord = { map = currentMap, x = 0, y = 0 },
                })
                table.insert(steps, {
                    type = "turnin", questID = questID, text = "Complete quest",
                    coord = { map = currentMap, x = 0, y = 0 },
                })
            ...
            local guide = {
                ...
                steps = steps,
                _imported = true, _source = "C_QuestLine",
            }
            TA.Guides[guideID] = guide
            count = count + 1
        end
